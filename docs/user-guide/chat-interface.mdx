...
<Tabs>
  ...
  <Tab title="📋 Planning Mode">
    **Strategic Planning** - Create detailed plans before execution ``` > /plan
    ...
    token management - User model and database schema - Authentication
    middleware ### Phase 2: API Endpoints - Registration endpoint - Login
    endpoint - Token refresh logic ### Phase 3: Frontend Integration -
    Login/register forms - Protected route wrapper - Authentication context Do
    you want to proceed with this plan? (y/n) ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Provide Context and Constraints">
    **Context Examples:** ``` > Following the same pattern as UserCard.tsx,
    create a ProductCard component with image, title, price, and add-to-cart
    functionality > Using our existing error handling pattern from
    authService.ts, add error handling to the payment processing flow > Without
    adding new dependencies, implement a simple modal system that works with our
    current React setup ``` **Benefits:** - Maintains code consistency -
    Respects existing patterns - Works within constraints
  </Accordion>
...
<Tabs>
  ...
  <Tab title="Conflict Resolution">
    ...
    Existing navigation vs. new navigation Options: 1. Keep existing and
    integrate new features 2. Replace with new implementation 3. Show both
    versions for manual merge Choose resolution strategy: (1/2/3) ```
  </Tab>
...
<CardGroup cols={2}>
  ...
  <Card title="Quick Actions" icon="bolt">
    **Tab**: Auto-complete commands/paths **Ctrl + L**: Clear screen **Ctrl +
    D**: Exit NikCLI
  </Card>
