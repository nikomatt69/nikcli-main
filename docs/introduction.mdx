...
<AccordionGroup>
  <Accordion title="Core Technologies">
    ...
    **Node.js 18+**: Runtime environment with latest features - **AI SDK**:
    Unified abstraction for multiple AI providers - **Vitest**: Test runner used
    for unit/integration tests
  </Accordion>

  <Accordion title="Terminal Interface">
    - **Chalk**: Colored output and visual hierarchy - **Boxen**: Structured
    information display - **Readline**: Interactive input with completion -
    **Ora**: Loading spinners and progress indicators
  </Accordion>

  <Accordion title="Development Tools">
    - **ESLint**: Code quality enforcement - **TypeScript Compiler**: Strict
    compilation settings - **Path Aliases**: Clean import structure - **Binary
    Distribution**: Cross-platform executables
  </Accordion>
</AccordionGroup>
...
<Steps>
  <Step title="Install NikCLI">
    ```bash npm install -g @nicomatt69/nikcli ```
  </Step>
  <Step title="Set API Key">
    ```bash export ANTHROPIC_API_KEY="your-api-key" ```
  </Step>
  <Step title="Start Interactive Mode">```bash nikcli ```</Step>
  <Step title="Try Your First Command">``` > /help ```</Step>
</Steps>
...
>
  Follow our comprehensive quickstart guide to set up NikCLI and start building
  with AI assistance.
</Card>
...
<CardGroup cols={3}>
  <Card
    ...
    icon="github"
    href="https://github.com/nikomatt69/agent-cli"
  >
