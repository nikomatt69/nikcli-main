...
class TaskExecutor {
        ...
        try {
            ...
            if (hasPackageJson) {
                ...
                try {
                    ...
                    if (await this.fileExists(owner, repo, 'bun.lockb'))
                        packageManager = 'bun';
                    else if (await this.fileExists(owner, repo, 'pnpm-lock.yaml'))
                        packageManager = 'pnpm';
                    else if (await this.fileExists(owner, repo, 'yarn.lock'))
                        packageManager = 'yarn';
                    else
                        packageManager = 'npm';
                    // Detect framework
    ...
    async cloneRepository(repoContext, branchName) {
        ...
        try {
            // Clone repository
            (0, node_child_process_1.execSync)(`git clone --depth 1 -b ${repoContext.defaultBranch} ${cloneUrl} ${repoContext.clonePath}`, { stdio: 'pipe' });
     ...
     */
    buildNikCLICommand(action, command, context) {
    ...
    extractAnalysisReport(output) {
        ...
        const lines = output.split('\n');
        const analysisLines = lines.filter(line => !line.includes('Loading') &&
