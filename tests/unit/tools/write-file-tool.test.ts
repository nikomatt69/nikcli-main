...
import { cleanup, fileExists, mockConsole, readFile } from '../../helpers/test-utils'
...
describe('WriteFileTool', () => {
  ...
  describe('Performance and Memory', () => {
    it('should handle concurrent writes efficiently', async () => {
      ...
      const results = await Promise.all(files.map((file) => writeFileTool.execute(file.path, file.content)))
    ...
    it('should not leak memory during operations', () => {
      ...
      const _tools = Array(10)
        .fill(null)
        .map(() => new WriteFileTool('/test'))
