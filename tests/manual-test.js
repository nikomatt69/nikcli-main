...
async function main() {
  ...
  total++
  if (
    await test('JSON Configuration', async () => {
      const config = {
  ...
  total++
  if (
    await test('Directory Operations', async () => {
  ...
  total++
  if (
    await test('Concurrent File Operations', async () => {
      const promises = []
      const fileCount = 10

      // Create multiple files concurrently
      for (let i = 0; i < fileCount; i++) {
        promises.push(fs.writeFile(`concurrent-${i}.txt`, `Content ${i}`))
      }
  ...
  total++
  if (
    await test('Data Processing', async () => {
      const inputData = {
        users: [
  ...
  total++
  if (
    await test('Security Validation', async () => {
      // Test path traversal detection
      const dangerousPaths = ['../../../etc/passwd', '..\\..\\..\\windows\\system32\\config', '/absolute/path/file.txt']
      ...
      // Test safe paths
      const safePaths = ['safe-file.txt', 'folder/safe-file.txt', 'deep/nested/safe-file.txt']
  ...
  if (
    await test('Performance Check', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        operations.push(fs.writeFile(`perf-${i}.txt`, `Performance test file ${i}`))
      }
  ...
  total++
  if (
    await test('Complete Workflow', async () => {
      ...
      const report = {
        ...
        analysis: {
          ...
          completionRate: Math.round(
            (loadedData.filter((i) => i.status === 'resolved').length / loadedData.length) * 100
          ),
        },
