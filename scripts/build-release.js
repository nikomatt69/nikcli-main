...
const builtFiles = fs.readdirSync(releasesDir).filter(file => 
    !file.endsWith('.json') && !file.startsWith('temp-')
);
...
fs.writeFileSync(
    path.join(completeArchiveDir, 'package.json'),
...
builtFiles.forEach(file => {
    ...
    fs.writeFileSync(
        path.join(platformArchiveDir, 'package.json'),
...
fs.writeFileSync(
    path.join(releasesDir, 'checksums.json'),
...
fs.writeFileSync(
    path.join(releasesDir, 'release-info.json'),
