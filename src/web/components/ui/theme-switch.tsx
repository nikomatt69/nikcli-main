...
export function ThemeSwitch({ className, showLabel = true, variant = 'toggle', size = 'default' }: ThemeSwitchProps) {
    ...
    return (
      ...
      >
        <div className="relative">
          ...
          {theme === 'system' && <Palette className="absolute -top-1 -right-1 h-2 w-2 text-muted-foreground" />}
        </div>
        {showLabel && <span className="text-foreground">{resolvedTheme === 'dark' ? 'Dark' : 'Light'}</span>}
      </button>
    )
  }
      ...
      >
        <div
          className={clsx(
            ...
            resolvedTheme === 'dark' ? 'translate-x-5 shadow-lg' : 'translate-x-0 shadow-md',
            sizes[size].thumb
          )}
        >
          {resolvedTheme === 'dark' ? (
