...
const InputGlass = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => {
  return <Input ref={ref} variant="glass" className={clsx('shadow-lg', className)} {...props} />
})
InputGlass.displayName = 'InputGlass'

const InputSearch = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => {
  return <Input ref={ref} variant="outline" className={clsx('pl-10', className)} {...props} />
})
