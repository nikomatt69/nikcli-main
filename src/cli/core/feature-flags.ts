...
export const FeatureFlagSchema = z.object({
  ...
  version: z.string().default('1.0.2'),
...
export class FeatureFlagManager extends EventEmitter {
  ...
  isEnabled(flagId: string): boolean {
    ...
    // Check user group
    if (!flag.userGroups.includes('all') && !flag.userGroups.includes(this.config.userGroup)) {
      return false
    }
  ...
  private async loadDefaultFlags(): Promise<void> {
    ...
    type FeatureFlagInit = Omit<
      z.input<typeof FeatureFlagSchema>,
