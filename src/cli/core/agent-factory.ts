...
import { z } from 'zod';
import { AgentSchema, AgentTaskResultSchema, type Agent } from '../schemas/core-schemas';
...
export const VMContainerConfigSchema = z.object({
  ...
  resourceLimits: z.object({
    memory: z.string().regex(/^\d+[GMK]?B?$/i).optional(),
    cpu: z.string().regex(/^\d+(\.\d+)?$/).optional(),
    disk: z.string().regex(/^\d+[GMK]?B?$/i).optional()
  }).optional(),
...
export class AgentFactory extends EventEmitter {
  ...
  private createFallbackBlueprint(specialization: string): any {
    ...
    const name = specialization.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').substring(0, 80);
...
Context Scope: ${requirements.contextScope || 'project'}`,
    ...
    try {
      ...
      const blueprintData = {
        id: nanoid(),
        name: requirements.name || aiBlueprint.name || requirements.specialization.toLowerCase().replace(/\s+/g, '-').substring(0, 80),
        ...
        personality: {
          proactive: Math.min(100, Math.max(0, aiBlueprint.personality?.proactive || requirements.personality?.proactive || 70)),
          collaborative: Math.min(100, Math.max(0, aiBlueprint.personality?.collaborative || requirements.personality?.collaborative || 60)),
          analytical: Math.min(100, Math.max(0, aiBlueprint.personality?.analytical || requirements.personality?.analytical || 80)),
          creative: Math.min(100, Math.max(0, aiBlueprint.personality?.creative || requirements.personality?.creative || 50)),
        },
...
Execute tasks step-by-step and verify results before proceeding.`;
    ...
    const blueprintData = {
      id: nanoid(),
      name: requirements.name || `${requirements.specialization.toLowerCase().replace(/\s+/g, '-').substring(0, 70)}-vm-agent`,
  ...
  async createAndLaunchAgent(requirements: Parameters<typeof this.createAgentBlueprint>[0], task?: string): Promise<DynamicAgent> {
