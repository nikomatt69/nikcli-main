...
export const ToolPermissionSchema = z.object({
  ...
  maxMemoryUsage: z.number().int().min(1024 * 1024).default(512 * 1024 * 1024),
...
export const ToolMetadataSchema = z.object({
  ...
  category: z.enum(['file-ops', 'code-analysis', 'system', 'network', 'development', 'utility', 'image-analysis', 'text-analysis', 'image-generation']),
...
export const ToolRegistryConfigSchema = z.object({
  enabledCategories: z.array(z.string()).default([
    'file-ops', 'code-analysis', 'system', 'network', 'development', 'utility', 'image-analysis', 'image-generation'
  ]),
...
export class ToolRegistry {
  ...
  private async registerBuiltInTools(): Promise<void> {
    const toolsDir = join(this.workingDirectory, 'src/cli/tools');
  ...
  private async validateTool(instance: any, metadata: ToolMetadata): Promise<void> {
    ...
    if (metadata.requiredCapabilities.length > 0) {
      const missingCapabilities = metadata.requiredCapabilities.filter(
  ...
  private async loadEssentialTools(): Promise<void> {
    const essentialTools = Array.from(this.tools.values()).filter(
      tool => tool.metadata.priority >= 90
    );
  ...
  private logRegistryStats(): void {
    ...
    if (stats.topTools.length > 0) {
      ...
      stats.topTools.forEach(tool => {
        console.log(chalk.gray(`     ${tool.name}: ${tool.usage} uses (${(tool.successRate * 100).toFixed(1)}% success)`));
      });
    }
  }
  ...
  private async performLSPContextAnalysis(toolInstance: ToolInstance, args: any[]): Promise<void> {
    try {
      ...
      if (insights.diagnostics.errors > 0) {
        advancedUI.logWarning(`⚠️  LSP found ${insights.diagnostics.errors} errors before ${toolInstance.metadata.name} execution`);
      }
