...
export class InputQueue {
   ...
   */
  enqueue(input: string, priority: 'high' | 'normal' | 'low' = 'normal', source: 'user' | 'system' | 'agent' = 'user'): string {
    ...
    advancedUI.logInfo(`📥 Input queued: ${input.substring(0, 30)}${input.length > 30 ? '...' : ''} (${this.queue.length} in queue)`);
  ...
  async processNext(processor: (input: string) => Promise<void>): Promise<QueuedInput | null> {
    ...
    try {
      advancedUI.logInfo(`🔄 Processing queued input: ${nextInput.input.substring(0, 30)}${nextInput.input.length > 30 ? '...' : ''}`);
      ...
      advancedUI.logSuccess(`✅ Queued input processed: ${nextInput.input.substring(0, 30)}${nextInput.input.length > 30 ? '...' : ''}`);
  ...
  showStats(): void {
    ...
    if (status.pendingInputs.length > 0) {
      ...
      status.pendingInputs.slice(0, 5).forEach((input, index) => {
        ...
        console.log(`${index + 1}. ${chalk.gray(timeAgo)} ${input.input.substring(0, 40)}${input.input.length > 40 ? '...' : ''}`);
      });
