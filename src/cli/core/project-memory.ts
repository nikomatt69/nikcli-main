...
const ProjectPreferences = z.object({
    // UI/UX Preferences
    ui: z.object({
...
export class ProjectMemoryManager {
            ...
            case 'command':
                ...
                if (!projectMemory.context.commonCommands.includes(action.details)) {
                    ...
                    if (projectMemory.context.commonCommands.length > 20) {
                        projectMemory.context.commonCommands =
                            projectMemory.context.commonCommands.slice(-20);
                    }
                }
                break;
            ...
            case 'file_edit':
                ...
                if (!projectMemory.context.frequentFiles.includes(action.details)) {
                    ...
                    if (projectMemory.context.frequentFiles.length > 30) {
                        projectMemory.context.frequentFiles =
                            projectMemory.context.frequentFiles.slice(-30);
                    }
                }
                break;
    ...
    public recordFeedback(
        ...
        if (projectMemory.learningData.userFeedback.length > 100) {
            projectMemory.learningData.userFeedback =
                projectMemory.learningData.userFeedback.slice(-100);
        }
     ...
     */
    public getSmartSuggestions(context: {
    ...
    public getProjectAnalytics(): {
        ...
        return {
            ...
            topFiles: this.getTopItems(projectMemory.context.frequentFiles).map(({ command, count }) => ({ file: command, count })),
