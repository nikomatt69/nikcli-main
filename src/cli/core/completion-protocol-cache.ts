...
export class CompletionProtocolCache {
  ...
  private generatePrefixKey(prefix: string): string {
    ...
    return crypto.createHash('md5').update(words.join(' ')).digest('hex').substring(0, 12)
  }
  ...
  private generateContextHash(context: string): string {
    ...
    return crypto.createHash('sha256').update(normalized.substring(0, 500)).digest('hex').substring(0, 16)
  }
