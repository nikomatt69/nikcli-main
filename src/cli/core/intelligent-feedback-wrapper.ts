...
export class IntelligentFeedbackWrapper {
  ...
  private async handleSuccessfulExecution(execution: ToolExecutionResult): Promise<void> {
    ...
    if (this.isSignificantSuccess(execution)) {
      await feedbackSystem.reportSuccess(toolName, context, `Successfully executed ${toolName} with positive outcome`, {
        agentType,
  ...
  private async handleGenericToolFeedback(execution: ToolExecutionResult): Promise<void> {
    ...
    if (executionTime > 10000) {
      // > 10 secondi
      await feedbackSystem.reportUsage(`slow_${toolName}`, `Tool ${toolName} took ${executionTime}ms to execute`, {
        agentType: execution.agentType,
        sessionId: execution.context,
    ...
    if (recentUsage > 5) {
      // Usato più di 5 volte di recente
    ...
    switch (toolName) {
      case 'smart_docs_search':
        alternatives.push(
          ...
          'Use /doc-add to add relevant documentation'
        )
        break
      case 'file_operations':
  ...
  private analyzePerformance(execution: ToolExecutionResult): void {
    ...
    const recentExecutions = this.executionHistory.filter((e) => e.toolName === toolName).slice(-10)
