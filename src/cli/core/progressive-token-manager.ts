 ...
 */
  ...
  async createProgressiveChunks(messages: CoreMessage[]): Promise<TokenChunk[]> {
      ...
      const content = typeof message.content === 'string'
        ...
        : Array.isArray(message.content)
          ? message.content.map((c: any) =>
            typeof c === 'object' && c.type === 'text' ? c.text : JSON.stringify(c)
          ).join('\n')
    ...
    console.log(chalk.blue(`📊 Created ${chunks.length} chunks, average ${Math.floor(messages.length / chunks.length)} messages per chunk`));
  ...
  async *processChunksProgressively(
    ...
    for (const chunk of chunks) {
      ...
      try {
        ...
        console.log(chalk.yellow(`⚙️ Processing chunk ${chunk.index + 1}/${totalChunks} (${chunk.estimatedTokens} tokens)...`));
  ...
  private async generateChunkSummary(chunk: TokenChunk): Promise<string> {
    ...
    const summary = `Chunk ${chunk.index}: ${chunk.messages.length} messages, ${chunk.estimatedTokens} tokens. ` +
  ...
  async compressMessages(messages: CoreMessage[]): Promise<CoreMessage[]> {
    ...
    const targetReduction = 1 - this.config.compressionRatio;
    ...
    console.log(chalk.blue(`📉 Compression: ${originalTokens} → ${compressedTokens} tokens (${Math.round(reduction * 100)}% reduction)`));
