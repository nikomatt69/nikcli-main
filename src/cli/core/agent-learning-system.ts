...
export class AgentLearningSystem {
  ...
  async recordDecision(
    ...
    if (outcome === 'failure' && error) {
      let failurePattern = learningData.failurePatterns.find(fp => 
        fp.tool === chosenTool && fp.error === error
      );
    ...
    learningData.successfulChoices = learningData.successfulChoices
      .slice(-100); // Last 100 per pattern
  ...
  private getBestTool(scores: Record<string, number>): {tool: string; score: number} | null {
    ...
    const [tool, score] = entries.reduce((best, current) => 
