...
import { TOKEN_LIMITS, calculateTokenCost, getModelPricing } from '../config/token-limits';
...
import { contextEnhancer, SmartContext, ContextSource } from './context-enhancer';
...
export class ContextManager {
  ...
  async optimizeContextAdvanced(
    ...
    const result: ContextOptimizationResult = {
      ...
      metrics: {
        ...finalMetrics,
        compressionRatio: (initialMetrics.estimatedTokens - finalMetrics.estimatedTokens) / initialMetrics.estimatedTokens,
  ...
  private extractKeywords(messages: CoreMessage[]): string[] {
    ...
    const importantWords = /\b(implement|create|fix|bug|error|file|function|class|method|variable|import|export|async|await|return|if|for|while|try|catch)\b/gi;
  ...
  private cacheOptimizationResult(key: string, result: ContextOptimizationResult): void {
    if (this.contextCache.size >= 50) { // Limit cache size
  ...
  async analyzeWorkspace(): Promise<ContextSummary> {
    ...
    try {
      ...
      if (workspaceInfo.relevantFiles && workspaceInfo.relevantFiles.length > 0) {
        ...
        summary.importantFiles = workspaceInfo.relevantFiles
          .slice(0, 10)
          .map(f => f.path);
      ...
      console.log(chalk.green(`✅ Workspace analyzed: ${summary.totalFiles} files, ${Object.keys(summary.languages).length} languages`));
  ...
  private compressContext(messages: CoreMessage[]): CoreMessage[] {
    ...
    for (const item of sortedByImportance) {
      if (allocatedTokens + item.tokens <= availableTokens * 0.8) { // Leave 20% buffer
  ...
  private calculateSemanticValue(message: CoreMessage): number {
    ...
    if (content.includes('?') || content.toLowerCase().includes('how') || content.toLowerCase().includes('what')) value += 0.2;
