...
import { commandPredictor, PredictionResult } from './command-predictor';
import { aiCompletionService, CompletionContext, AICompletion } from '../services/ai-completion-service';
...
export class SmartCompletionManager {
   ...
   */
  private async getMLCompletions(
    partialInput: string,
    context?: any
  ): Promise<UnifiedCompletion[]> {
    try {
  ...
  private initializeStaticCommands(): void {
    ...
    const defaultCommands = [
      '/help', '/status', '/agents', '/clear', '/exit', '/models', '/config'
    ];

    // Orchestrator service commands
    const orchestratorCommands = [
      ...defaultCommands,
      '/diff', '/accept', '/middleware'
    ];

    // Autonomous interface commands
    const autonomousCommands = [
      ...defaultCommands,
      '/add-dir', '/analyze', '/auto', '/bug', '/cd', '/compact', '/cost',
      '/doctor', '/export', '/history', '/ls', '/model', '/pwd', '/autonomous',
      '/context', '/plan', '/auto-accept', '/diff', '/accept', '/reject', '/quit'
    ];

    // Unified chat commands
    const unifiedChatCommands = [
      '/help', '/plan', '/status', '/queue', '/stop', '/clear', '/exit'
    ];
  ...
  private mlCategoryToUnified(mlCategory: string): 'command' | 'parameter' | 'path' | 'agent' | 'tool' | 'code' | 'natural' {
  ...
  public formatCompletionsForDisplay(completions: UnifiedCompletion[]): string[] {
    return completions.map(comp => {
      ...
      if (color === 'cyan') return chalk.cyan(`${icon} ${comp.completion}` + (comp.description ? chalk.gray(` - ${comp.description}`) : '') + (comp.source === 'ai' ? chalk.dim(` (${confidence}%)`) : ''));
      if (color === 'green') return chalk.green(`${icon} ${comp.completion}` + (comp.description ? chalk.gray(` - ${comp.description}`) : '') + (comp.source === 'ai' ? chalk.dim(` (${confidence}%)`) : ''));
      if (color === 'blue') return chalk.blue(`${icon} ${comp.completion}` + (comp.description ? chalk.gray(` - ${comp.description}`) : '') + (comp.source === 'ai' ? chalk.dim(` (${confidence}%)`) : ''));
      if (color === 'yellow') return chalk.yellow(`${icon} ${comp.completion}` + (comp.description ? chalk.gray(` - ${comp.description}`) : '') + (comp.source === 'ai' ? chalk.dim(` (${confidence}%)`) : ''));
      if (color === 'magenta') return chalk.magenta(`${icon} ${comp.completion}` + (comp.description ? chalk.gray(` - ${comp.description}`) : '') + (comp.source === 'ai' ? chalk.dim(` (${confidence}%)`) : ''));
      
      return chalk.white(`${icon} ${comp.completion}` + (comp.description ? chalk.gray(` - ${comp.description}`) : '') + (comp.source === 'ai' ? chalk.dim(` (${confidence}%)`) : ''));
    });
  }
