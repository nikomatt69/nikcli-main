...
export class ToolRouter extends EventEmitter {
  ...
  private toolKeywords: ToolKeyword[] = [
    ...
    {
      tool: 'glob_tool',
      keywords: ['glob', 'pattern', 'glob pattern', '*.ts', '*.json', '**/*', 'globby', 'fast glob', 'glob search'],
      priority: 6,
      description: 'Fast glob pattern matching with sorting and filtering',
      examples: ['glob **/*.ts', 'find all *.json files', 'glob pattern with filters'],
    },
    {
      tool: 'diff_tool',
      keywords: ['diff', 'compare', 'comparison', 'changes', 'difference', 'unified diff', 'side by side'],
      priority: 5,
      description: 'Compare files and content with multiple diff algorithms',
      examples: ['diff file1.ts file2.ts', 'compare two files', 'show differences'],
    },
    {
      tool: 'tree_tool',
      keywords: ['tree', 'directory tree', 'structure', 'visualize', 'directory structure', 'folder tree'],
      priority: 4,
      description: 'Visualize directory structure as a tree',
      examples: ['show directory tree', 'tree structure of src/', 'visualize folder'],
    },
    {
      tool: 'watch_tool',
      keywords: ['watch', 'monitor', 'file watch', 'file system watch', 'monitor changes', 'file events'],
      priority: 3,
      description: 'Monitor file system changes in real-time',
      examples: ['watch src/ for changes', 'monitor file changes', 'watch directory'],
    },
