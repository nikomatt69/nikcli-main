...
export class UnifiedTokenBudget {
  ...
  getBudgetStats(): {
    ...
    const totalAllocated = Array.from(this.allocations.values()).reduce((sum, entry) => sum + entry.allocated, 0)
  ...
  optimizeAllocation(): void {
    ...
    if (avgUsage > this.strategy.baseAllocation * 0.8) {
      this.strategy.baseAllocation = Math.min(this.strategy.maxTokensPerTask, Math.floor(avgUsage * 1.2))
    } else if (avgUsage < this.strategy.baseAllocation * 0.3) {
      this.strategy.baseAllocation = Math.max(1000, Math.floor(avgUsage * 1.5))
    }
  }
  ...
  getEfficiencyMetrics(): {
    ...
    const completedTasks = Array.from(this.allocations.values()).filter((entry) => entry.used > 0)

    const totalWaste = completedTasks.reduce((sum, entry) => sum + entry.remaining, 0)

    const totalAllocated = completedTasks.reduce((sum, entry) => sum + entry.allocated, 0)

    const wastePercentage = totalAllocated > 0 ? (totalWaste / totalAllocated) * 100 : 0
    const averageUtilization =
      completedTasks.length > 0
        ? completedTasks.reduce((sum, entry) => sum + entry.used / entry.allocated, 0) / completedTasks.length
        : 0
