...
export class IDEAwareFormatter {
  ...
  static formatEnvironmentContext(): string {
    ...
    lines.push(chalk.cyan(`  * Editor: ${caps.name === 'vscode' || caps.name === 'cursor' ?
      caps.name.toUpperCase() + ' (GUI IDE detected)' :
      caps.name === 'terminal' ? 'Terminal/CLI (no GUI IDE open)' :
  ...
  static createPrompt(context: {
    ...
    const ideIndicator = this.createStatusIndicator();
