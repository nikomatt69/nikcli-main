...
export class AdvancedCliUI {
  ...
  private printIndicatorLine(indicator: StatusIndicator): void {
    ...
    const _statusColor = this.getStatusColor(indicator.status)
  ...
  showGrepResults(pattern: string, matches: any[]): void {
    const grepContent = matches
      .map((match) => {
        ...
        const line = (match.content || match.match || '').replace(new RegExp(pattern, 'gi'), chalk.bgYellow.black('$&'))
  ...
  createPanel(panelConfig: StructuredPanel): void {
    ...
    const withDefaults: StructuredPanel = {
      pinned: panelConfig.id === 'todos' ? true : defaultPinned,
      priority: panelConfig.id === 'todos' ? 100 : 0,
      ...panelConfig,
      borderColor: color,
    }
  ...
  private buildDiffSummary(filePath: string, oldContent: string, newContent: string): string {
    ...
    lines.push('')
    lines.push(
      `${chalk.green(`+${added} additions`)} ${chalk.gray('|')} ${chalk.red(`-${removed} deletions`)} ${chalk.gray('|')} ${chalk.white(`${totalNew} lines total`)}`
    )
    lines.push(
      `${chalk.gray('Change')} ${'█'.repeat(Math.max(1, Math.round(changePct / 5)))}${'░'.repeat(Math.max(0, 20 - Math.round(changePct / 5)))} ${chalk.yellow(`${changePct}%`)}`
    )
  ...
  private updateAgentsPanel(): void {
    ...
    const content = agents
      .map((agent) => {
        ...
        const timeInfo = agent.startTime ? ` (${this.formatDuration(Date.now() - agent.startTime.getTime())})` : ''
