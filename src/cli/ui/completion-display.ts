...
import { UnifiedCompletion } from '../core/smart-completion-manager';
...
export class CompletionDisplay {
  ...
  private formatCompactCompletion(comp: UnifiedCompletion, index: number): string {
    ...
    if (this.options.showDescription && comp.description) {
      ...
      const desc = comp.description.length > maxDescLength 
  ...
  private colorizeText(text: string, color: ChalkColor, style?: 'bold' | 'underline'): string {
    ...
    switch (color) {
      case 'cyan': chalkFn = chalk.cyan; break;
      case 'green': chalkFn = chalk.green; break;
      case 'yellow': chalkFn = chalk.yellow; break;
      case 'blue': chalkFn = chalk.blue; break;
      case 'magenta': chalkFn = chalk.magenta; break;
      case 'red': chalkFn = chalk.red; break;
      case 'white': chalkFn = chalk.white; break;
      case 'gray': chalkFn = chalk.gray; break;
      case 'grey': chalkFn = chalk.grey; break;
      default: chalkFn = chalk.white; break;
    }
  ...
  createFooter(): string {
    const tips = [
      chalk.dim('Tab: Accept • Ctrl+C: Cancel'),
      chalk.dim('↑/↓: Navigate • Enter: Execute')
    ];
