...
import { SecureVirtualizedAgent, VMState } from '../virtualized-agents/secure-vm-agent';
import { AgentSession } from '../virtualized-agents/security/api-key-proxy';
...
export class VMStatusIndicator extends EventEmitter {
  ...
  private formatAgentStatus(agentId: string, status: VMAgentStatus): string {
    ...
    // Main status line
    lines.push(`${stateIcon} ${chalk.bold(truncatedId)} ${chalk.dim(`(${status.vmState})`)} - ${this.formatUptime(uptime)}`);
