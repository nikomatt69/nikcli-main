...
import { join, relative, resolve, extname, dirname, basename } from 'path';
...
class ProjectDetector {
  ...
  detectLintTool(): DetectedTool {
    // Check for ESLint
    if (existsSync(join(this.workingDir, '.eslintrc.js')) ||
    ...
    if (existsSync(join(this.workingDir, 'pyproject.toml')) ||
      existsSync(join(this.workingDir, 'requirements.txt'))) {
      return {
        name: 'ruff',
  ...
  detectTestTool(): DetectedTool {
    ...
    // Check for Pytest
    if (existsSync(join(this.workingDir, 'pytest.ini')) ||
      existsSync(join(this.workingDir, 'pyproject.toml'))) {
      return {
        name: 'pytest',
...
class DiagnosticParser {
  ...
  static parseGenericBuild(output: string, stderr: string, exitCode: number): { diagnostics: Diagnostic[], summary: BuildSummary } {
    const diagnostics: Diagnostic[] = [];
...
export class IDEDiagnosticServer extends EventEmitter {
  ...
  private readonly allowedCommands = new Set([
    ...
    'tsc', 'eslint', 'biome', 'vitest', 'jest',
    'python', 'pytest', 'ruff', 'cargo', 'go',
  ...
  async diagList(filters: {
    ...
    // Collect from cache
    for (const [key, diagnostics] of this.cache['cache'].entries()) {
  ...
  async diagGet(file: string, line?: number): Promise<{
    ...
    if (line !== undefined) {
      diagnostics = fileDiagnostics.filter(d =>
        d.range && d.range.startLine <= line && d.range.endLine >= line
      );
    }
  ...
  async lintRun(): Promise<{ summary: { errors: number; warnings: number; files: number }; diagnostics: Diagnostic[] }> {
  ...
  async vcsStatus(): Promise<VcsStatus> {
    ...
    try {
      ...
      try {
        const statusResult = await this.executeCommand(['git', 'status', '--porcelain=v1', '--branch'], { cwd: this.workingDir });
  ...
  async runtimeLogs(service?: string, lines: number = 100, cursor?: string): Promise<{
    ...
    try {
      const watcher = require('chokidar').watch(pathToWatch, {
        ignored: [
          ...
          'target', 'bin', 'obj', '.cache', '.temp', 'tmp',
          '*.log', '.DS_Store', 'Thumbs.db', '.vscode',
          '.idea', '*.swp', '*.swo', '.nyc_output', 'coverage',
          '.pytest_cache', '__pycache__', '*.pyc', '.mypy_cache',
          'vendor', '.bundle', '.sass-cache', '.npm'
        ],
        ignoreInitial: true
      });
