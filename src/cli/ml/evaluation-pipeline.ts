...
class EvaluationPipeline {
  ...
  async runBatchEvaluation(lookbackDays: number = 7): Promise<EvaluationMetrics> {
    ...
    try {
      ...
      const executions = await this.supabaseProvider.getToolchainExecutions(
        startDate
      );
  ...
  private calculateMetrics(
    ...
    const totalDuration = executions.reduce(
      (sum, e) => sum + (e.execution_duration_ms || 0),
      0
    );
  ...
  private async getHistoricalMetrics(): Promise<HistoricalMetrics | null> {
    ...
    try {
      ...
      // Calculate trend
      const accuracyTrend =
        current.toolSelectionAccuracy - baseline.toolSelectionAccuracy;
      const successTrend =
        current.executionSuccessRate - baseline.executionSuccessRate;
  ...
  private detectRegressions(
    ...
    if (
      ...
      historical.baseline.toolSelectionAccuracy - threshold
    ) {
      regressions.push(
        `Tool selection accuracy regressed to ${(metrics.toolSelectionAccuracy * 100).toFixed(1)}%`
      );
    }

    if (
      metrics.executionSuccessRate <
      historical.baseline.executionSuccessRate - threshold
    ) {
      regressions.push(
        `Execution success rate regressed to ${(metrics.executionSuccessRate * 100).toFixed(1)}%`
      );
    }

    if (
      metrics.averageExecutionDuration >
      historical.baseline.averageExecutionDuration * 1.2
    ) {
      regressions.push(
        `Execution duration increased by 20%+ (${metrics.averageExecutionDuration.toFixed(0)}ms)`
      );
    }
  ...
  private detectImprovements(
    ...
    if (
      ...
      const improvement = (
        (metrics.toolSelectionAccuracy /
          historical.baseline.toolSelectionAccuracy -
          1) *
        100
      ).toFixed(1);
      improvements.push(
        `Tool selection accuracy improved by ${improvement}%`
      );
    }
    ...
    if (
      ...
      const improvement = (
        (metrics.executionSuccessRate /
          historical.baseline.executionSuccessRate -
          1) *
        100
      ).toFixed(1);
    ...
    if (
      ...
      (historical.baseline.cacheHitRate || 0) + threshold
    ) {
      improvements.push(
        `Cache hit rate improved to ${(metrics.cacheHitRate * 100).toFixed(1)}%`
      );
    }
  ...
  private generateRecommendations(
    ...
    if (metrics.toolSelectionAccuracy < 0.75) {
      recommendations.push(
        'Consider collecting more training data for improved tool selection'
      );
    }
    ...
    if (metrics.averageExecutionDuration > 5000) {
      recommendations.push(
        'Implement tool sequencing optimization to reduce execution time'
      );
    }
    ...
    if (metrics.cacheHitRate < 0.3) {
      recommendations.push(
        'Review cache invalidation strategy to improve cache effectiveness'
      );
    }
    ...
    if (metrics.modelConfidenceScore < 0.7) {
      recommendations.push(
        'Increase model training data diversity to improve prediction confidence'
      );
    }
    ...
    if (failedTools.length > 0) {
      recommendations.push(
        `Review reliability of tools: ${failedTools.slice(0, 3).join(', ')}`
      );
    }
  ...
  private async storeBatchMetrics(
    metrics: EvaluationMetrics,
    lookbackDays: number
  ): Promise<void> {
    if (!this.supabaseProvider) {
