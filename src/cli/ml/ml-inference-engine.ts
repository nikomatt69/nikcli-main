...
class MLInferenceEngine {
  ...
  async predictSuccessRates(tools: string[], context: Record<string, any>): Promise<Record<string, number>> {
    if (!this.initialized) {
      return {}
    }
  ...
  async optimizeSequence(patterns: Record<string, any>): Promise<SequenceOptimization> {
    ...
    try {
      const successfulCombinations = patterns.successfulToolCombinations || {}
      ...
      const potentialImprovement = this.calculatePerformanceImprovement(recommendedSequence, patterns)

      // Build caching strategy
      const cachingStrategy = this.buildCachingStrategy(recommendedSequence)
  ...
  private scoreToolCombinations(featureVector: FeatureVector): Map<string, number> {
    ...
    const toolScores: Record<string, number> = {
      ...
      'edit-tool': intentType.includes('edit') || intentType.includes('modify') ? 95 : 65,
      'glob-tool': intentType.includes('search') || intentType.includes('find') ? 90 : 75,
      'grep-tool': intentType.includes('search') || intentType.includes('find') ? 92 : 73,
  ...
  private getContextAdjustment(tool: string, context: Record<string, any>): number {
    let adjustment = 0
  ...
  private generateReasoningText(tools: string[], featureVector: FeatureVector): string {
    if (tools.length === 0) {
  ...
  private calculatePerformanceImprovement(sequence: string[], patterns: Record<string, any>): number {
  ...
  private buildCachingStrategy(sequence: string[]): Record<string, number> {
  ...
  private async getInferenceCache(intentHash: string): Promise<ToolPredictionResult | null> {
  ...
  private async cacheInference(intentHash: string, prediction: ToolPredictionResult, ttl: number): Promise<void> {
    if (!this.cacheService) {
      return
    }
...
export type { FeatureVector, ToolPredictionResult, SuccessPrediction, SequenceOptimization }
