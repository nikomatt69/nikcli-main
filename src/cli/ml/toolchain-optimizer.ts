...
class ToolchainOptimizer extends EventEmitter {
  ...
  async optimizeToolchainPerformance(executions: ToolExecution[]): Promise<ToolchainOptimization> {
  ...
  private async checkAndTriggerRetraining(metrics: ModelMetrics): Promise<void> {
    ...
    try {
      ...
      const improvement = (metrics.accuracy - baselineAccuracy) / Math.max(baselineAccuracy, 0.01)

      if (improvement > this.RETRAINING_THRESHOLD) {
        structuredLogger.info(
          'ToolchainOptimizer',
          `Triggering model retraining (${(improvement * 100).toFixed(2)}% improvement)`
        )
