...
class FeatureExtractor {
  ...
  private extractFeatures(
    ...
    if (context.performanceMetrics) {
      const perfFeatures = this.extractPerformanceFeatures(
  ...
  private extractContextFeatures(context: Record<string, any>): Record<string, number> {
    ...
    if (context.fileTypes && Array.isArray(context.fileTypes)) {
      ...
      features['file_type_diversity'] = Math.min(
        context.fileTypes.length / 10,
        1
      );
    }
    ...
    if (context.projectSize) {
      features['project_size_normalized'] = Math.min(
        context.projectSize / 1000,
        1
      );
    }
    ...
    if (context.sessionLength) {
      features['session_length_normalized'] = Math.min(
        context.sessionLength / 3600,
        1
      );
    }
  ...
  private extractHistoryFeatures(
    toolSuccessHistory: Record<string, any>
  ): Record<string, number> {
    const features: Record<string, number> = {};
  ...
  private extractPerformanceFeatures(
    ...
    if (performanceMetrics.latency) {
      features['avg_latency_normalized'] = Math.min(
        performanceMetrics.latency.avg / 5000,
        1
      );
      features['p95_latency_normalized'] = Math.min(
        performanceMetrics.latency.p95 / 10000,
        1
      );
    }
    ...
    if (performanceMetrics.cache) {
      ...
      features['cache_memory_usage_normalized'] = Math.min(
        performanceMetrics.cache.memoryUsed / 100000000,
        1
      );
    }
    ...
    if (performanceMetrics.tokens) {
      features['tokens_per_session_normalized'] = Math.min(
        performanceMetrics.tokens.total / 100000,
        1
      );
    }
  ...
  private estimateDifficulty(userIntent: string): number {
    ...
    if (
      userIntent.match(/refactor|optimize|analyze|complex|multiple|batch/i)
    ) {
      difficulty += 0.2;
    }
