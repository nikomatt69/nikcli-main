...
import { NikEnvironment, EnvironmentPolicies, BackgroundJob, HeadlessOptions } from '../types';
...
export class SecurityPolicy {
  ...
  static validateCommand(
    ...
    if (policies.allowedCommands && policies.allowedCommands.length > 0) {
      const commandAllowed = policies.allowedCommands.some(allowed => 
   ...
   */
  static createSecureHeadlessOptions(
