...
`
  ...
  private async createPullRequest(job: BackgroundJob, workspaceDir: string, containerId: string): Promise<void> {
    ...
    try {
      ...
      // Commit changes - use file-based commit message to avoid quote issues
      const commitMessage = `feat: ${job.task.replace(/"/g, '\\"')}

🤖 Generated by NikCLI Background Agent
⏰ ${new Date().toISOString()}`

      // Write commit message to file to avoid shell quote issues
      const fs = await import('node:fs/promises')
      const path = await import('node:path')
      const commitMsgFile = path.join(repoDir, '.git', 'COMMIT_EDITMSG')
      await fs.writeFile(commitMsgFile, commitMessage, 'utf8')

      execSync('git commit -F .git/COMMIT_EDITMSG', { cwd: repoDir })
