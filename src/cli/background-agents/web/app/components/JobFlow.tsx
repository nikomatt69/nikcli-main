          ...
          return (
            <div key={step.id} className="flex items-start gap-4">
              ...
              <div className="flex-1 pt-2">
                ...
                {isActive && <p className="text-sm text-gray-600 dark:text-gray-400">In progress...</p>}
                {isCompleted && <p className="text-sm text-green-600 dark:text-green-400">✓ Complete</p>}
                {(isFailed || isTimeout) && (
                  <p className="text-sm text-red-600 dark:text-red-400">{job.error || 'Failed'}</p>
                )}
              </div>
            </div>
          )
        ...
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900 dark:text-white">{job.metrics.tokenUsage.toLocaleString()}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-gray-900 dark:text-white">{job.metrics.memoryUsage.toFixed(0)} MB</p>
            </div>
          </div>
        </div>
      )}
