...
import { z } from 'zod';
import { NikEnvironment, TerminalConfig, EnvironmentPolicies } from '../types';
...
export class EnvironmentParser {
   ...
   */
  static async createDefault(workingDirectory: string, packageManager: 'yarn' | 'npm' | 'pnpm' = 'yarn'): Promise<void> {
  ...
  static mergeWithOverrides(
    environment: NikEnvironment,
    overrides: Partial<NikEnvironment>
  ): NikEnvironment {
    return {
      ...environment,
      ...overrides,
