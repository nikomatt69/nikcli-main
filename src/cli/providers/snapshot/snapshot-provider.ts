...
export class SnapshotProvider extends EventEmitter {
  ...
  constructor() {
    ...
    this.config = {
      ...
      compression: true,
      maxSnapshots: 50,
      excludePatterns: ['node_modules/**', '.git/**', 'dist/**', 'build/**', '*.log', '.env*', '*.tmp'],
    }
  ...
  async listSnapshots(options: SnapshotSearchOptions = {}): Promise<SnapshotEntry[]> {
    ...
    if (options.author) {
      results = results.filter((s) => s.metadata.author === options.author)
    }
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          try {
            const content = await fs.readFile(path.join(this.config.localPath!, file), 'utf8')
