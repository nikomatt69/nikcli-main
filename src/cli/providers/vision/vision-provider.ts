...
export class VisionProvider extends EventEmitter {
  ...
  private async analyzeWithClaude(
    ...
    const apiKey = simpleConfigManager.getApiKey('claude-3-5-sonnet-20241022') || simpleConfigManager.getApiKey('anthropic');
    ...
    const systemPrompt = customPrompt || `Analyze this image in detail and provide a comprehensive analysis. Focus on:
...
Provide structured, detailed insights that would be useful for understanding the image content and context.`;
  ...
  private async analyzeWithOpenAI(
    ...
    const systemPrompt = customPrompt || `Analyze this image comprehensively and provide structured insights in the following areas:
...
Provide thorough, useful insights for understanding the image content and context.`;
  ...
  private async analyzeWithGoogle(
    ...
    const systemPrompt = customPrompt || `Provide a comprehensive analysis of this image covering:
...
Deliver detailed, structured insights for complete image understanding.`;
  ...
  getAvailableProviders(): string[] {
    ...
    // Check Anthropic
    try {
      const anthropicKey = simpleConfigManager.getApiKey('claude-3-5-sonnet-20241022') || simpleConfigManager.getApiKey('anthropic');
  ...
  updateConfig(newConfig: Partial<VisionConfig>): void {
    ...
    console.log(chalk.blue('👁️ Vision Provider configuration updated'));
