...
export const docsRequestTool: CoreTool = tool({
  ...
  execute: async ({ concept, context, urgency, autoLoad, suggestSources }) => {
    try {
      ...
      const relevantLoaded = currentDocs.filter(
        (doc) =>
      ...
      if (result.found) {
        ...
        await feedbackSystem.reportSuccess(concept, context, `Found ${loaded + available} relevant documents`, {
        ...
        await feedbackSystem.reportDocGap(concept, context, impactLevel as 'low' | 'medium' | 'high', 'first-time', {
          agentType: 'docs_request',
...
export const docsGapReportTool: CoreTool = tool({
  ...
  parameters: z.object({
    ...
    frequency: z
      ...
      .describe('How often this gap is encountered'),
    impact: z.enum(['low', 'medium', 'high', 'critical']).describe('How much this impacts your ability to help users'),
    suggestedSources: z
      .array(z.string())
      .optional()
      .describe('URLs or sources where this documentation might be found'),
  ...
  execute: async ({ missingConcept, frequency, impact, suggestedSources, userContext }) => {
    try {
      ...
      await feedbackSystem.reportDocGap(missingConcept, userContext, impact, frequency, {
      ...
      const summary =
        `${priorityLevel} Documentation Gap Reported\n\n` +
