...
export const createSnapshotTool = tool({
  description: 'Create a snapshot of the current project state. Supports different types: quick (current state), full (entire project), dev (source code only), config (configuration files)',
...
export const restoreSnapshotTool = tool({
  description: 'Restore files from a snapshot. Can restore to current directory or specified path, with options for overwrite and file selection',
...
export const compareSnapshotsTool = tool({
  ...
  execute: async ({ snapshot1Id, snapshot2Id }) => {
    try {
      console.log(chalk.blue(`🔍 Comparing snapshots: ${snapshot1Id.substring(0, 8)}... vs ${snapshot2Id.substring(0, 8)}...`));
