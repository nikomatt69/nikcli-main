...
export class ReplaceInFileTool extends BaseTool {
  ...
  async replaceWithContext(
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const hasMatch = typeof searchPattern === 'string' ? line.includes(searchPattern) : searchPattern.test(line)
   ...
   */
  private generatePreview(originalContent: string, newContent: string, maxLines: number = 10): ChangePreview {
