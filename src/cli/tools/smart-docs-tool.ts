...
export const smartDocsSearchTool: CoreTool = tool({
  ...
  parameters: z.object({
    ...
    urgency: z
      ...
      .default('medium')
      .describe('How critical this information is for the current task'),
  }),
  ...
  execute: async ({ includeContent, suggestForQuery }) => {
    try {
      ...
      if (!result.hasContext) {
        summary =
          'No documentation currently loaded in context. Use the documentation search tool to find and load relevant docs.'
      } else {
