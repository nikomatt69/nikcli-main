...
export class PlanGenerator {
  ...
  private assessPlanRisk(steps: ExecutionStep[]) {
    ...
    const fileModifications = steps.filter(
      (s) => s.toolName?.includes('write') || s.toolName?.includes('delete') || s.toolName?.includes('replace')
    ).length
  ...
  private generatePlanDescription(analysis: RequestAnalysis, steps: ExecutionStep[]): string {
    ...
    const riskLevel = steps.some((s) => s.riskLevel === 'high')
      ? 'high'
