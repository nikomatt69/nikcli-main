...
import { JobStatus, BackgroundJob } from '../background-agents/types';
...
export class BackgroundAgentsCommand {
  ...
  static register(program: Command): void {
    const bg = program
      .command('bg')
      .alias('background')
      .description('Manage background agents and jobs');
  ...
  private static async sendFollowUp(jobId: string, message: string, options: any): Promise<void> {
    ...
    try {
      const messageId = await backgroundAgentService.sendFollowUpMessage(
        jobId,
        message,
        options.priority
      );
