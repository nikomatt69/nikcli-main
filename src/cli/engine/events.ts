...
export async function runAnalysisWithEvents(
  ...
  try {
    ...
    const globby =
      (await import('globby')).globby ||
      (await import('globby')).default ||
      (await import('globby')).globbySync;
    ...
    files
      .slice(0, 200)
      .forEach((f) =>
        emitter.emitEvent({ type: 'fileQueued', payload: { file: f } }),
      );
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      findings.push({
        ...
        severity: 'low',
        description:
          'Ensure build/test scripts run in CI and cover all packages.',
    ...
    if (fs.existsSync(readme)) {
      ...
      findings.push({
        ...
        severity: 'info',
        description:
          'Project documentation is present. Consider adding badges and usage examples.',
