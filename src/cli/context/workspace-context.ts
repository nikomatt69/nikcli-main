...
export class WorkspaceContextManager {
  ...
  private async initializeRAGIntegration(): Promise<void> {
    try {
      ...
      console.log(chalk.green(`✅ RAG integration ${this.context.ragAvailable ? 'enabled' : 'disabled (fallback mode)'}`));
    } catch (error) {
  ...
  getContextForAgent(agentId: string, maxFiles: number = 20, searchQuery?: string): {
    selectedPaths: string[];
  ...
  private getFilteredContextForAgent(agentId: string, maxFiles: number = 20, searchQuery?: string): {
    selectedPaths: string[];
    relevantFiles: FileContext[];
    projectSummary: string;
    totalContext: string;
  } {
    console.log(chalk.yellow(`🔍 Auto-filtering context${searchQuery ? ` for query: "${searchQuery}"` : ' (large workspace)'}...`));
