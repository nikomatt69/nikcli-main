...
export class DocsContextManager {
  ...
  private buildSmartQuery(projectSummary: string): string {
    const frameworks = ['react', 'vue', 'angular', 'next'].find(f =>
      projectSummary.toLowerCase().includes(f)
    );
    const languages = ['typescript', 'javascript', 'python'].find(l =>
      projectSummary.toLowerCase().includes(l)
    );
  ...
  getContextStats(): {
    ...
    const hitRate = this.analytics.cacheHits + this.analytics.cacheMisses > 0
