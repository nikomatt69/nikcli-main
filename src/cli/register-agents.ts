...
export function registerAgents(agentManager: AgentManager): void {
  ...
  agentManager.registerAgentClass(UniversalAgent, {
    ...
    category: 'sub-agent',
    tags: ['sub-agent', 'intelligent', 'autonomous', 'production-ready'],
    requiresGuidance: false,
    defaultConfig: {
      autonomyLevel: 'fully-autonomous',
      maxConcurrentTasks: 3,
      ...
      enabledTools: ['file', 'terminal', 'git', 'npm', 'analysis', 'docker', 'kubernetes', 'deployment', 'infrastructure', 'monitoring', 'security'],
      guidanceFiles: [],
      logLevel: 'info',
      permissions: {
        canReadFiles: true,
