...
export class AICompletionService {
  ...
  private async enrichContext(context: CompletionContext): Promise<any> {
    ...
    try {
      ...
      enriched.projectType = contextData.projectSummary.includes('React') ? 'web' : 
                           contextData.projectSummary.includes('Python') ? 'python' :
                           contextData.projectSummary.includes('Node') ? 'web' : 'general';
    ...
    // Add available tools
    enriched.availableTools = toolService.getAvailableTools().map(t => t.name).slice(0, 20);
    ...
    if (context.openFiles && context.openFiles.length > 0) {
      try {
        ...
        const currentFileData = contextData.relevantFiles.find(f => 
          context.openFiles!.includes(f.path)
        );
        if (currentFileData) {
...
Provide 3-8 most relevant completions, ranked by relevance and confidence.`;
  ...
  private parseAIResponse(response: string, context: any): AICompletion[] {
    try {
      ...
      return completions.map((comp: any) => {
        ...
        return completion;
      }).filter((comp: AICompletion) => 
        comp.completion && 
        comp.completion.trim() !== context.partialInput &&
        comp.completion.length > 0
      );
    } catch (error) {
   ...
   */
  private validateCategory(category: string): 'command' | 'parameter' | 'path' | 'agent' | 'tool' | 'code' | 'natural' | null {
  ...
  private shouldRequireApproval(completion: string): boolean {
    const dangerousPatterns = [
      /^rm\s/, /^delete\s/, /^drop\s/, /^truncate\s/, 
      /^kill\s/, /^sudo\s/, /^chmod\s.*777/
    ];
