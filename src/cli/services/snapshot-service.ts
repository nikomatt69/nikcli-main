...
import { snapshotProvider, SnapshotEntry, SnapshotSearchOptions } from '../providers/snapshot/snapshot-provider';
...
export class SnapshotService extends EventEmitter {
   ...
   */
  async cleanupSnapshots(criteria: {
  ...
  private scheduleTemplate(name: string, template: SnapshotTemplate): void {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const snapshotId = await this.createFromTemplate(
          name, 
        ...
        if (template.schedule!.maxKeep) {
          ...
          const templatedSnapshots = scheduledSnapshots.filter(s => 
            s.metadata.tags.includes(name)
          );
