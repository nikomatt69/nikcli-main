...
import { mem0Provider, MemoryEntry, MemorySearchOptions, MemorySearchResult } from '../providers/memory';
...
export class MemoryService extends EventEmitter {
  ...
  async startSession(
    ...
    await this.addMemory(
      `Started conversation session${options.topic ? ` about ${options.topic}` : ''}`,
      {
        source: 'system',
  ...
  async getConversationContext(
    ...
    const memories = await mem0Provider.getMemoriesByContext(
      targetSessionId,
      { limit: 50, userId: this.currentSession?.userId }
    );
  ...
  async getRelevantContext(
    ...
    const personalization = targetUserId ? 
      await this.getPersonalization(targetUserId) : null;
  ...
  async endSession(): Promise<void> {
    ...
    await this.addMemory(
      `Ended conversation session (duration: ${Math.round(duration / 60000)} minutes)`,
      {
        source: 'system',
        context: 'session_end',
        importance: 3,
  ...
  private async analyzeUserPersonalization(
    ...
    const preferredLength = avgWordsPerMessage < 10 ? 'short' : 
                           avgWordsPerMessage < 30 ? 'medium' : 'long';
    ...
    return {
      userId,
      preferences: {},
      expertise_areas: frequentTopics.filter(topic => 
        ['programming', 'database', 'api', 'design'].includes(topic)
      ),
      communication_style: 'balanced',
  ...
  private async incrementalPersonalizationUpdate(
