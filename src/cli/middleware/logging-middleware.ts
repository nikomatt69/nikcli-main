...
export class LoggingMiddleware extends BaseMiddleware {
  ...
  private logRequest(request: MiddlewareRequest): void {
    const sanitizedRequest = this.loggingConfig.sanitizeData 
  ...
  private logResponse(
    ...
    const sanitizedResponse = this.loggingConfig.sanitizeData 
    ...
    if (this.shouldLogToConsole('info')) {
      ...
      console.log(
        `${statusIcon} [${request.id.slice(0, 8)}] ${request.operation} ` +
  ...
  private logError(
    ...
    if (this.shouldLogToConsole('error')) {
      console.log(
        chalk.red(`❌ [${request.id.slice(0, 8)}] ${request.operation} `) +
  ...
  private logPerformance(
    ...
    if (duration > 5000 && this.shouldLogToConsole('warn')) {
      console.log(
        chalk.yellow(`⚠️ [${request.id.slice(0, 8)}] SLOW OPERATION: `) +
        `${request.operation} took ${duration}ms`
      );
    }
  }
