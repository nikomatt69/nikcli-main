...
export class AuditMiddleware extends BaseMiddleware {
  ...
  private async createAuditEntry(params: {
    ...
    const signature = this.auditConfig.enableIntegrityChecks 
      ? await this.signAuditEntry(baseEntry) 
      : undefined;
  ...
  private assessOperationRisk(request: MiddlewareRequest): 'low' | 'medium' | 'high' {
    ...
    const highRiskOperations = [
      'delete', 'remove', 'rm', 'sudo', 'su', 'chmod 777',
      'system administration', 'production', 'deploy',
      'database migration', 'security', 'credentials'
    ];

    const mediumRiskOperations = [
      'write', 'edit', 'create', 'modify', 'git push',
      'npm install', 'yarn add', 'pip install',
      'docker run', 'file manipulation'
    ];
  ...
  private async detectChanges(
    ...
    if (request.type === 'file') {
      if (['write', 'edit', 'create', 'modify'].some(op => 
        request.operation.toLowerCase().includes(op))) {
  ...
  private checkCompliance(
    ...
    const sensitiveDataDetected = sensitiveDataPatterns.some(pattern => 
      pattern.test(combinedData)
    );
  ...
  async generateComplianceReport(): Promise<ComplianceReport> {
    ...
    const complianceScore = totalEntries > 0 ? 
      ((gdprCompliantEntries / totalEntries) * 100) : 100;
