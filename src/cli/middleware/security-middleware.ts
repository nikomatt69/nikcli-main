...
export class SecurityMiddleware extends BaseMiddleware {
  ...
  constructor(policyManager: ExecutionPolicyManager, config: Partial<SecurityMiddlewareConfig> = {}) {
  ...
  private async performSecurityCheck(request: MiddlewareRequest): Promise<{
    ...
    if (this.securityConfig.blockedOperations.some((blocked) => operation.includes(blocked.toLowerCase()))) {
      return {
        allowed: false,
    ...
    if (
      this.securityConfig.allowedOperations.length > 0 &&
  ...
  private assessFileOperationRisk(request: MiddlewareRequest): 'low' | 'medium' | 'high' {
    ...
    const mediumRiskFiles = ['.js', '.ts', '.py', '.sh', '.bash', '.zsh']
  ...
  private logSecurityEvent(operation: string, riskLevel: string, action: string, reason: string): void {
    const event = {
      timestamp: new Date(),
      operation,
      riskLevel,
