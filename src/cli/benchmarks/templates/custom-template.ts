    ...
    async evaluateOutput(
        ...
        if (criteria) {
            ...
            if (criteria.requiredPatterns) {
                const matched = criteria.requiredPatterns.filter((pattern: string) =>
                ...
                if (patternScore < 1) {
                    const missing = criteria.requiredPatterns.filter((pattern: string) =>
                        !new RegExp(pattern, 'i').test(output)
                    )
            ...
            if (criteria.forbiddenPatterns) {
                const found = criteria.forbiddenPatterns.filter((pattern: string) =>
                    new RegExp(pattern, 'i').test(output)
                )
                if (found.length > 0) {
