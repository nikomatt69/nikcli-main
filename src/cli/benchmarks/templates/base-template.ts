...
import type { BenchmarkTask, BenchmarkTemplate, TaskResult } from '../types'
...
export abstract class BaseBenchmarkTemplate {
   ...
   */
  abstract loadTasks(options?: { limit?: number; difficulty?: string; tags?: string[] }): Promise<void>
  ...
  protected extractCode(text: string, language?: string): string {
    const pattern = language ? new RegExp(`\`\`\`${language}\\n([\\s\\S]*?)\`\`\``, 'g') : /```[\w]*\n([\s\S]*?)```/g
