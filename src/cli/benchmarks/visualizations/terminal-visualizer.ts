...
import type { BenchmarkSession, BenchmarkMetrics } from '../types'
...
export class TerminalVisualizer {
	...
	showSummary(session: BenchmarkSession): void {
		...
		console.log(chalk.white(`  Tasks: ${m.success.total} total | ${chalk.green(m.success.passed + ' passed')} | ${chalk.red(m.success.failed + ' failed')}`))
		console.log(chalk.white(`  Success Rate: ${m.success.rate >= 0.8 ? chalk.green : m.success.rate >= 0.5 ? chalk.yellow : chalk.red}(${(m.success.rate * 100).toFixed(2)}%)`))
		console.log('')

		console.log(chalk.cyan.bold('⚡ Performance:'))
		console.log(chalk.white(`  Latency: avg ${chalk.yellow(m.latency.avg.toFixed(2) + 'ms')} | p50 ${m.latency.p50.toFixed(2)}ms | p95 ${m.latency.p95.toFixed(2)}ms`))
		console.log(chalk.white(`  Tokens: ${chalk.yellow(m.tokens.total.toLocaleString())} total | ${m.tokens.avgPerTask.toFixed(0)} avg/task`))
		console.log(chalk.white(`  Cost: ${chalk.yellow('$' + m.cost.total.toFixed(4))} total | $${m.cost.avgPerTask.toFixed(4)} avg/task`))
		console.log('')

		console.log(chalk.cyan.bold('🎯 Quality:'))
		console.log(chalk.white(`  Accuracy: ${chalk.yellow((m.accuracy.avg * 100).toFixed(2) + '%')} avg | ${m.accuracy.min * 100}% min | ${m.accuracy.max * 100}% max`))
		console.log(chalk.white(`  Errors: ${m.errors.total} (${(m.errors.rate * 100).toFixed(2)}%)`))
		console.log('')

		console.log(chalk.cyan.bold('💾 Resources:'))
		console.log(chalk.white(`  Memory: ${chalk.yellow(this.formatBytes(m.resources.memoryPeak))} peak | ${this.formatBytes(m.resources.memoryAvg)} avg`))
		console.log(chalk.white(`  CPU: ${chalk.yellow(m.resources.cpuPeak.toFixed(2) + '%')} peak | ${m.resources.cpuAvg.toFixed(2)}% avg`))
		console.log('')
