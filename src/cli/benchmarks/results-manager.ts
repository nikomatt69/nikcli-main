...
import type { BenchmarkComparisonResult, BenchmarkSession } from './types'
...
export class ResultsManager {
  ...
  async saveSession(session: BenchmarkSession): Promise<string> {
    ...
    await fs.writeFile(path.join(sessionDir, 'metrics.json'), JSON.stringify(session.metrics, null, 2))

    // Save task results
    await fs.writeFile(path.join(sessionDir, 'tasks.json'), JSON.stringify(session.tasks, null, 2))
   ...
   */
  async compareSessions(sessionId1: string, sessionId2: string): Promise<BenchmarkComparisonResult> {
...
${
  Object.entries(m.errors.byType).length > 0
    ? '\n**By Type**:\n' +
      Object.entries(m.errors.byType)
...
${session.tasks
  ...
  .map(
    (t) =>
      `| ${t.taskId} | ${t.success ? '✅' : '❌'} | ${t.executionTime.toFixed(2)} | ${t.tokensUsed.total} | $${t.cost.toFixed(4)} | ${t.accuracy ? (t.accuracy * 100).toFixed(2) + '%' : 'N/A'} |`
  )
  .join('\n')}
