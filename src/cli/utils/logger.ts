...
export class Logger {
  ...
  private logToConsole(entry: LogEntry): void {
    ...
    console.log(
      `${chalk.gray(timestamp)} ${icon} ${colorFunc(level)} ${entry.message}`
    );
...
export const logTask = (level: LogLevel, taskId: string, agentId: string, message: string, context?: Record<string, any>) =>
...
export const audit = (action: string, context: Record<string, any>) =>
