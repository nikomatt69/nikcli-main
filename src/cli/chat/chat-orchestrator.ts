...
import type { AgentTodoManager } from '../core/agent-todo-manager'
import type { SimpleConfigManager } from '../core/config-manager'
import { GuidanceManager } from '../guidance/guidance-manager'
import type { ChatMessage, SessionData, SessionManager } from '../persistence/session-manager'
import type { Agent, AgentStatus } from '../types/types'
...
export class ChatOrchestrator {
  ...
  async handleInput(sessionId: string, input: string): Promise<void> {
    try {
      ...
      session.messages.push({
        role: 'assistant',
        content: `Planned ${todos.length} tasks:\n${summary}`,
        timestamp: new Date().toISOString(),
      })
      await this.sessionManager.saveSession(session)
      ...
      this.log('error', 'Error handling input', {
        sessionId,
        error: error instanceof Error ? error.message : String(error),
      })
...
/guidance stats - Show guidance statistics`
      ...
      case '/sessions':
        ...
        session.messages.push({
          role: 'assistant',
          content: list.length ? list.map((s) => s.id).join(',') : 'None',
          timestamp: new Date().toISOString(),
        })
        break
      case '/config':
        ...
        session.messages.push({
          role: 'assistant',
          content: 'Configuration displayed in console',
          timestamp: new Date().toISOString(),
        })
        break
      case '/guidance':
    ...
    switch (subCommand) {
      ...
      case 'create':
        if (args.length < 3) {
          session.messages.push({
            role: 'assistant',
            content: 'Usage: /guidance create <type> <location>\nType: claude|codex|agents\nLocation: global|project',
            timestamp: new Date().toISOString(),
          })
          return
        }
        ...
        if (!['claude', 'codex', 'agents'].includes(type)) {
          session.messages.push({
            role: 'assistant',
            content: 'Invalid type. Use: claude, codex, or agents',
            timestamp: new Date().toISOString(),
          })
          return
        }
        ...
        if (!['global', 'project'].includes(location)) {
          session.messages.push({
            role: 'assistant',
            content: 'Invalid location. Use: global or project',
            timestamp: new Date().toISOString(),
          })
          return
        }
...
Last Updated: ${context?.lastUpdated ? new Date(context.lastUpdated).toLocaleString() : 'Never'}`
      ...
      default:
        session.messages.push({
          role: 'assistant',
          content: `Unknown guidance command: ${subCommand}. Use /help for available commands.`,
          timestamp: new Date().toISOString(),
        })
    }
  }
  ...
  private validateCommandStrict(originalCommand: string): void {
    const normalize = (cmd: string): string => {
      ...
      const strippedQuotes = cmd.trim().replace(/["'\u2018\u2019\u201C\u201D]/g, '')
    ...
    const prohibitedExecutables =
      /^(rm|sudo|chmod|chown|curl|wget|dd|mkfs|fdisk|mount|umount|kill|killall|systemctl|service|crontab|at|batch|scp|ssh|rsync)$/i
