...
const CommandArgsSchema = z.array(z.string());
...
const CreateAgentCommandSchema = z.object({
  name: z.string().min(3).max(90).regex(/^[a-zA-Z0-9-]+$/, "Name must contain only alphanumeric characters and hyphens"),
...
const FileCommandSchema = z.object({
  path: FilePathSchema,
...
${chalk.gray('New: Use /ui-kit enable for modern terminal UI experience!')}
  ...
  private async searchCommand(args: string[]): Promise<CommandResult> {
    if (args.length === 0) {
      ...
      console.log(chalk.gray('  /search --web <query> [--type general|technical|documentation|stackoverflow] [--mode results|answer] [--includeContent] [--maxContentBytes N] [--maxResults N]'));
      return { shouldExit: false, shouldUpdatePrompt: false };
    }
    ...
    if (hasWebFlag) {
      try {
        ...
        if (!query) {
          console.log(chalk.red('Usage: /search --web <query> [--type ...] [--mode ...] [--includeContent] [--maxContentBytes N] [--maxResults N]'));
          return { shouldExit: false, shouldUpdatePrompt: false };
        }
    ...
    try {
      ...
      if (blueprints.length > 0) {
        ...
        blueprints.forEach((blueprint, index) => {
          ...
          console.log(`   Capabilities: ${blueprint.capabilities.slice(0, 3).join(', ')}${blueprint.capabilities.length > 3 ? '...' : ''}`);
  ...
  private getMatchReason(query: string, blueprint: any): string {
    ...
    const matchingCaps = blueprint.capabilities.filter((cap: string) =>
  ...
  private async analyzeImageCommand(args: string[]): Promise<CommandResult> {
    if (args.length === 0) {
      ...
      console.log(chalk.cyan('/analyze-image <path> --provider <claude|openai|google|vercel>') + ' - Use specific provider');
  ...
  private async generateImageCommand(args: string[]): Promise<CommandResult> {
    if (args.length === 0) {
      ...
      console.log(chalk.cyan('/generate-image "prompt" --model <dall-e-3|dall-e-2|gpt-image-1>') + ' - Use specific model');
  ...
  private async snapshotCommand(args: string[]): Promise<CommandResult> {
    ...
    try {
      ...
      console.log(chalk.cyan(`📊 Total snapshots: ${stats.totalSnapshots}, Total size: ${this.formatSize(stats.totalSize)}`));
  ...
  private async listSnapshotsCommand(args: string[]): Promise<CommandResult> {
    ...
    try {
      ...
      snapshots.forEach(snapshot => {
        ...
        console.log(chalk.gray(`   ID: ${id} | Created: ${created} | Size: ${size} | Files: ${snapshot.metadata.fileCount}`));
      ...
      // Show stats
      const stats = snapshotService.getSnapshotStats();
      console.log(chalk.cyan(`📊 Total: ${stats.totalSnapshots} snapshots, ${this.formatSize(stats.totalSize)} total size`));
  ...
  private async showPersonalization(): Promise<void> {
    ...
    console.log(chalk.cyan(`Preferred Response Length: ${personalization.interaction_patterns.preferred_response_length}`));
    ...
    if (personalization.interaction_patterns.common_tasks.length > 0) {
      console.log(chalk.cyan(`Common Tasks: ${personalization.interaction_patterns.common_tasks.slice(0, 3).join(', ')}`));
    }
  }
  ...
  private async indexCommand(args: string[]): Promise<CommandResult> {
    ...
    try {
      ...
      console.log(chalk.cyan('\n🤖 RAG System Status:'));
      console.log(`  Vector DB: ${(ragStats as any).vectorDBAvailable ? chalk.green('Available') : chalk.yellow('Unavailable')}`);
      console.log(`  Workspace RAG: ${(ragStats as any).workspaceRAGAvailable ? chalk.green('Available') : chalk.yellow('Unavailable')}`);
