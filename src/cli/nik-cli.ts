    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const { enhancedPlanning } = await import('./planning/enhanced-planning').catch(() => ({
            enhancedPlanning: {
              generatePlan: async () => ({
                id: 'fallback-plan',
                title: 'Fallback Plan',
                description: 'Generated by fallback planning system',
                todos: [],
                estimatedTotalDuration: 300000,
                riskAssessment: { overallRisk: 'low', destructiveOperations: 0, fileModifications: 0, externalCalls: 0 }
              })
            }
          }))
      ...
      if (approved) {
        ...
        try {
          ...
          const { approvalSystem } = await import('./ui/approval-system').catch(() => ({
            approvalSystem: {
              confirm: async () => false,
              promptInput: async () => ''
            }
