...
import { clearInterruptFlag, isInterruptRequested, setInterruptFlag } from './utils/interrupt-flag'
// VM System imports
...
export class NikCLI {
  ...
  private async startEnhancedChat(): Promise<void> {
    ...
    this.rl?.on('line', async (input) => {
      ...
      // If paste is pending submission, only accept empty input (Enter key)
      if (this.pastePendingSubmit) {
        if (input.trim() === '') {
          // User pressed Enter - submit the stored paste content
          const pendingPaste = this.consumePendingPaste()
          if (pendingPaste) {
            // RESUME PROMPT and disable bypass before processing (same pattern as /theme create)
            this.resumePromptAndRender()
            inputQueue.disableBypass()
            await this.processSingleInput(pendingPaste)
          }
        }
        // Non-empty input or auto-generated line event - ignore
        return
      }
  ...
  private setupBracketedPasteInterception(): void {
    ...
    process.stdin.on('data', (chunk: Buffer) => {
      ...
      // Suspend queue immediately when paste starts
      if (this.pasteHandler.isPasting() && !this.isQueueSuspended) {
        this.suspendInputQueue()
      }
      ...
      // Capture pre-paste content from passthrough (text before paste marker)
      const existingContent = result.passthrough || ''

      if (result.isPasteComplete && result.pastedContent) {
        // BLOCK: Prevent readline line events from processing passthrough
        // Set flag SYNCHRONOUSLY to block any pending line events
        this.pastePendingSubmit = true
      ...
      // Note: We block line events by setting pastePendingSubmit synchronously.
      // The line handler will ignore any events generated by readline for passthrough.
    })
  }
  ...
  private isQueueSuspended: boolean = false // Flag to block queue during paste operations
  private pastePendingSubmit: boolean = false // Flag: paste complete, waiting for Enter

  /**
   * Suspend input queue during paste operations
   */
  public suspendInputQueue(): void {
    this.isQueueSuspended = true
  }

  /**
   * Resume input queue after paste operations complete
   */
  public resumeInputQueue(): void {
  ...
  private async handleConsolidatedPaste(content: string, existingContent: string = ''): Promise<void> {
    try {
      ...
      if (!trimmed) {
        // Empty paste - resume queue, let normal line handling take over
        this.resumeInputQueue()
        return
      }
      ...
      if (!pasteResult.shouldTruncate) {
        // Short text that wasn't detected as paste - resume queue
        this.resumeInputQueue()
        return
      }

      // Remove any individual lines that might have been queued during paste
      inputQueue.removeByContent(content)
      ...
      this.pastePendingSubmit = true // Block auto line events until Enter pressed

      // SUSPEND PROMPT and enable bypass (same pattern as /theme create)
      this.suspendPrompt()
      inputQueue.enableBypass()

      // Clear the line (which now contains existing + pasted text visible)
      // Then write the indicator followed by the prompt on the next line
      if (this?.rl) {
        // Clear current line first to avoid duplication
        this.rl.write('', { ctrl: true, name: 'u' })
        // Write the indicator as the current line content
        this.rl?.write(existingContent + pasteResult.displayText)
      }

      // Render prompt below the indicator - input is still blocked by pastePendingSubmit
      this.renderPromptAfterOutput()

      // Do NOT process yet - wait for Enter key (handled by 'line' event)
      // Prompt is now visible with indicator, input is blocked until Enter is pressed
    } catch {
      // On error, resume queue to avoid getting stuck
      this.resumeInputQueue()
    }
  }
  ...
  private consumePendingPaste(): string | null {
    if (this.pendingPasteContent) {
      ...
      this.pastePendingSubmit = false // Reset flag to allow normal input after paste
      // isQueueSuspended stays true until paste processing completes in _executeInput()
      return content
    }
    return null
  }
  ...
  private async _executeInput(actualInput: string, optimizedInput: string): Promise<void> {
    ...
    try {
      ...
      this.renderPromptAfterOutput()
      // Resume input queue after processing (including pastes)
      if (this.isQueueSuspended) {
        this.resumeInputQueue()
      }
...
- Each agent must have clear, actionable specialization`
      ...
      try {
        // Check for interrupt before AI call
  ...
  private renderContextProgressBar(width: number = 10, compact: boolean = false): string {
    ...
    if (compact) {
      ...
      // Standard format
      return color(
        `Context: ${bar} ${percentage.toFixed(0)}% `
      )
    }
  }
...
`
  ...
  /**
   * Open interactive command palette modal with search (Ctrl/Cmd+B)
   */
  private async openCommandPaletteModal(): Promise<void> {
    ...
    // Flatten groups for easier processing
    const allCommands = this.flattenGroupsForPalette(slashGroups)
    ...
    const render = () => {
      const panel = this.buildCommandPalettePanel(
        searchQuery,
        visibleCommands,
        selectedIndex,
        scrollOffset,
    ...
    const handleKey = (data: Buffer): boolean => {
      ...
      if (key === '\u001b') { // Escape
        ...
        if (visibleCommands.length > 0 && selectedIndex < visibleCommands.length) {
          selectedCommand = visibleCommands[selectedIndex].cmd
        }
        shouldExit = true
        return true
        ...
        if (selectedIndex < visibleCommands.length - 1) selectedIndex++
        updateScroll()
        render()
      } else if (key === '\u007f' || key === '\b') { // Backspace
        if (searchQuery.length > 0) {
          searchQuery = searchQuery.slice(0, -1)
          visibleCommands = this.filterCommands(allCommands, searchQuery)
          selectedIndex = 0
          scrollOffset = 0
          render()
        }
      } else if (key.length === 1 && key >= ' ' && key.charCodeAt(0) < 127) { // Printable ASCII
        searchQuery += key
        visibleCommands = this.filterCommands(allCommands, searchQuery)
        selectedIndex = 0
        scrollOffset = 0
  ...
  /**
   * Flatten groups for palette display
   */
  private flattenGroupsForPalette(
    groups: { title: string; commands: [string, string][] }[]
  ): Array<{ cmd: string; desc: string; group: string }> {
    const result: Array<{ cmd: string; desc: string; group: string }> = []
    for (const group of groups) {
      for (const [cmd, desc] of group.commands) {
        result.push({ cmd, desc, group: group.title })
      }
    }
    return result
  }
  ...
  private filterCommands(
    allCommands: Array<{ cmd: string; desc: string; group: string }>,
    query: string
  ): Array<{ cmd: string; desc: string; group: string }> {
    ...
    return allCommands.filter(
      item =>
        item.cmd.toLowerCase().includes(lowerQuery) ||
  ...
  /**
   * Build the command palette panel with boxen
   */
  private buildCommandPalettePanel(
    searchQuery: string,
    ...
    // Search bar
    const searchPlaceholder = chalk.dim('Search commands...')
    const searchContent = searchQuery
      ? chalk.white(searchQuery)
      : searchPlaceholder
    const searchBar = chalk.bgHex('#2d2d2d').white(` Search: ${searchContent} `)

    // Filtered commands list (only visible portion)
    const commandList = this.renderCommandList(commands, selectedIndex, scrollOffset, maxVisible)
    ...
    const footer =
      ...
      chalk.cyan(`${selectedIndex + 1}/${commands.length}`)

    return boxen(
      [searchBar, '', commandList, '', footer].join('\n'),
      {
        width: PANEL_WIDTH,
        padding: 1,
        margin: 0,
  ...
  /**
   * Render the command list with group headers (paginated)
   */
  private renderCommandList(
    ...
    for (const item of visibleCommands) {
      const actualIndex = scrollOffset + visibleCount

      if (item.group !== currentGroup) {
        // Close previous group if it had visible items
        if (groupStarted && groupHasVisibleItems) {
          result += '\n'
        }
        ...
        const cleanGroup = this.stripEmojis(item.group)
        result += chalk.yellow(`[${cleanGroup}]`) + '\n'
        result += chalk.hex('#30363d')('─'.repeat(54)) + '\n'
      }

      const isSelected = actualIndex === selectedIndex
      const prefix = isSelected
        ? chalk.bgHex('#58a6ff').black(' > ')
        : chalk.dim('   ')
      const cmdStyle = isSelected
        ? chalk.bgHex('#58a6ff').black.bold(item.cmd)
        : chalk.cyan(item.cmd)
      const descStyle = isSelected
        ? chalk.dim(item.desc)
        : chalk.dim(item.desc)

      result += `${prefix} ${cmdStyle}  ${descStyle}\n`
