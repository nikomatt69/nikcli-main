  ...
  private async executeToolInVM(toolName: string, params: any, originalInput: string): Promise<void> {
    ...
    try {
      ...
      const executionPromise = (async () => {
        const chunks = vmOrchestrator.executeCommandStreaming(this.activeVMContainer, vmCommand)
...
old_escaped = re.sub(r'([.+*?^${}()|[\\]\\\\])', r'\\\\\\1', '''${params.old_string.replace(/'/g, "'\"'\"'")}''')
new_content = re.sub(old_escaped, '''${params.new_string.replace(/'/g, "'\"'\"'")}''', content, flags=re.MULTILINE | re.DOTALL)
...
for edit in edits:
    ...
    try:
        ...
        if edit.get('old_string') and edit.get('new_string'):
            old_escaped = re.sub(r'([.+*?^${}()|[\\]\\\\])', r'\\\\\\1', edit['old_string'])
            content = re.sub(old_escaped, edit['new_string'], content, flags=re.MULTILINE | re.DOTALL)
...
EOF`
  ...
  private async executeCommandInVM(command: string): Promise<void> {
    ...
    try {
      ...
      // Execute in VM container with streaming output
      const executionPromise = (async () => {
        const chunks = vmOrchestrator.executeCommandStreaming(this.activeVMContainer, command)
