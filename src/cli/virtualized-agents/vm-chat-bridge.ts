...
import { vmSessionManager, VMSession } from './vm-session-manager';
...
export class VMChatBridge extends EventEmitter implements VMEventEmitter {
    ...
    try {
      ...
      if (!agent) {
        throw new VMCommunicationError(
          `VM agent ${agentId} not found or not registered`,
          agentId,
          'AGENT_NOT_FOUND'
        );
      }
      ...
      if (!containerId) {
        throw new VMCommunicationError(
          `VM agent ${agentId} has no active container`,
          agentId,
          'NO_CONTAINER'
        );
      }
  ...
  async broadcastMessage(message: string): Promise<VMBridgeResponse[]> {
    const promises = Array.from(this.activeAgents.keys()).map(agentId =>
  ...
  private async handleChatMessage(message: VMChatMessage): Promise<void> {
    ...
    if (agent) {
      console.log(chalk.blue(`📨 Received chat message from agent ${agent.id}: ${message.payload.content.slice(0, 50)}...`));
