...
export class TokenManager extends EventEmitter {
  ...
  getAgentTokens(agentId: string): TokenInfo[] {
    return Array.from(this.activeTokens.values()).filter((token) => token.agentId === agentId && !token.revoked)
  }
  ...
  getAllActiveTokens(): TokenInfo[] {
    return Array.from(this.activeTokens.values()).filter((token) => !token.revoked)
  }
  ...
  getTokenStatistics(): TokenStatistics {
    ...
    return {
      ...
      oldestToken: activeTokens.length > 0 ? Math.min(...activeTokens.map((t) => t.issuedAt.getTime())) : null,
      newestToken: activeTokens.length > 0 ? Math.max(...activeTokens.map((t) => t.issuedAt.getTime())) : null,
    }
  }
