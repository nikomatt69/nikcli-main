...
export class VMSessionManager extends EventEmitter implements VMEventEmitter {
  ...
  async createSession(
    containerId: string,
    agentId: string,
    config?: Partial<VMSessionConfig>
  ): Promise<VMSession> {
  ...
  async sendChatMessage(
    ...
    if (!session || !session.isActive) {
      throw new VMSessionError(
        `Session ${sessionId} not found or inactive`,
        sessionId,
        'invalid_state'
      );
    }
    ...
    try {
      const message = createVMChatMessage(
        session.containerId,
        sessionId,
        content,
        sender
      ...
      if (vmWebSocketServer.isConnected(session.containerId)) {
        ...
        console.log(chalk.blue(`💬 Sent chat message to ${session.containerId.slice(0, 12)}: ${content.slice(0, 50)}...`));
      } else {
  ...
  private async saveSessionHistory(session: VMSession): Promise<void> {
    ...
    console.log(chalk.cyan(`💾 Would save session history for ${session.sessionId} (${session.history.messages.length} messages)`));
  }
  ...
  private setupEventHandlers(): void {
    ...
    vmWebSocketServer.on('disconnected', (containerId: string) => {
      const session = this.getSessionByContainer(containerId);
      if (session) {
        console.log(chalk.yellow(`📵 Container ${containerId.slice(0, 12)} disconnected, session ${session.sessionId} now offline`));
      }
    });
