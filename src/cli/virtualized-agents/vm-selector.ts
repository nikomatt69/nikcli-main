...
import { SecureVirtualizedAgent } from './secure-vm-agent';
...
export class VMSelector {
  ...
  private chatHistory: Map<string, Array<{
    role: 'user' | 'vm' | 'system';
    content: string;
    timestamp: Date;
  }>> = new Map();
  ...
  async getAvailableVMs(options: VMSelectorOptions = {}): Promise<VMTarget[]> {
    try {
      ...
      for (const container of containers) {
        ...
        // Apply filters
        if (options.filterByRepository && 
            !target.repositoryUrl?.includes(options.filterByRepository)) {
          continue;
        }
  ...
  async selectVM(options: VMSelectorOptions = {}): Promise<VMTarget | null> {
    ...
    if (!this.chatHistory.has(this.selectedVM!.id)) {
      ...
      await this.addSystemMessage(this.selectedVM!.id, 
  ...
  async showVMDashboard(): Promise<void> {
    ...
    vms.forEach(vm => {
      ...
      if (vm.systemInfo) {
        console.log(`   System: ${vm.systemInfo.os} ${vm.systemInfo.arch}  Working Dir: ${vm.systemInfo.workingDirectory}`);
      }
      
      if (vm.resourceUsage) {
        console.log(`   Resources: Memory ${vm.resourceUsage.memory}  CPU ${vm.resourceUsage.cpu}  Disk ${vm.resourceUsage.disk}`);
      }
      
      const lastActivity = vm.lastActivity ? 
        `${Math.round((Date.now() - vm.lastActivity.getTime()) / 1000 / 60)}m ago` : 'Never';
  ...
  private formatVMChoice(vm: VMTarget): string {
    ...
    const repo = vm.repositoryUrl ? 
      chalk.gray(`(${vm.repositoryUrl.split('/').slice(-1)[0]})`) : '';
  ...
  private async getVMResourceUsage(containerId: string): Promise<any> {
    try {
      ...
      const { stdout } = await execAsync(`docker stats ${containerId.slice(0, 12)} --no-stream --format "table {{.MemUsage}}\\t{{.CPUPerc}}"`);
  ...
  async showVMSystemStatus(vmId?: string): Promise<void> {
    const targetVM = vmId ? 
      this.vmOrchestrator.getActiveContainers().find(c => c.id === vmId) :
      this.selectedVM;
  ...
  async performHealthCheckAll(): Promise<void> {
    ...
    if (critical > 0) {
      ...
      healthResults.filter(r => r.health === 'critical').forEach(result => {
  ...
  async getVMSessionStats(): Promise<void> {
    ...
    vmStats.forEach(stat => {
      ...
      console.log(chalk.gray(`   Last Activity: ${typeof stat.lastActivity === 'object' ? stat.lastActivity.toLocaleString() : stat.lastActivity}`));
      console.log();
    });
  }
