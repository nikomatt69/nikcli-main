...
import { EventBus } from '../../automation/agents/event-bus';
import { EnhancedGitHubService } from '../github/github-service';
import { EnhancedSlackService } from '../slack/slack-service';
...
export class IntegrationOrchestrator {
  ...
  private setupEventListeners(): void {
    ...
    this.eventBus.on(
      ...
      this.handleGitHubEvent.bind(this),
    );
  ...
  private async handleGitHubEvent(event: WorkflowEvent): Promise<void> {
    ...
    const matchingWorkflows = this.findWorkflowsByTrigger(
      'github_event',
      event.type,
    );
  ...
  private findWorkflowsByTrigger(
    triggerType: string,
    eventType: string,
  ): WorkflowDefinition[] {
    return Array.from(this.workflows.values()).filter(
      (workflow) =>
        ...
        workflow.triggers.some(
          (trigger) =>
            trigger.type === triggerType && trigger.event === eventType,
        ),
    );
  }
  ...
  private async evaluateCondition(
    ...
    const template = condition.field.includes('{{')
      ? condition.field
      : `{{${condition.field}}}`;
    const fieldValue = this.resolveTemplateValue(
      template,
      event,
      (event as any).actionResults ?? [],
    );
  ...
  private async executeAction(
    ...
    const expandedParams = this.expandTemplateParameters(
      action.parameters,
      event,
      previousResults,
    );
  ...
  private expandTemplateParameters(
    ...
    for (const [key, value] of Object.entries(parameters)) {
      if (typeof value === 'string' && value.includes('{{')) {
        ...
        expanded[key] = this.expandTemplateParameters(
          value,
          event,
          previousResults,
        );
  ...
  private async executeGitHubAction(
    ...
    switch (operation) {
      ...
      case 'createIssue':
        return await this.githubService.createIntelligentIssue(
          repoParams.owner,
          repoParams.repo,
          params.data,
        );
      case 'commentOnPR':
        return await this.githubService.processPullRequest(
          repoParams.owner,
          repoParams.repo,
  ...
  private async executeSlackAction(
    ...
    switch (operation) {
      ...
      case 'notifyDeploymentStatus':
        return await this.slackService.notifyDeploymentStatus(
          params.channel ?? this.config.devChannel,
          params.status,
        );
      case 'sendErrorAlert':
        return await this.slackService.sendErrorAlert(
          params.channel ?? this.config.devChannel,
          params.error,
        );
      default:
  ...
  private async executeNikCLIAction(
    operation: string,
    params: any,
  ): Promise<any> {
    ...
    switch (operation) {
      case 'runAnalysis':
        return await this.taskMaster.generateTasksWithAI(
          `Analyze ${params.target} for ${params.purpose}`,
        );
