...
import type { SlackCommand, SlackConfig as SlackConfigType, SlackMessage } from '../types'
...
export class EnhancedSlackService {
  ...
  async notifyRepositoryAnalysis(
    ...
    const template: NotificationTemplate = {
      ...
      color: analysis.score > 80 ? 'good' : analysis.score > 60 ? 'warning' : 'danger',
      fields: [
        {
          title: 'Score',
  ...
  async notifyDeploymentStatus(
    ...
    const template: NotificationTemplate = {
      title: `${emojis[status.status]} Deployment ${
        status.status === 'success' ? 'Completed' : status.status === 'failure' ? 'Failed' : 'In Progress'
      }`,
  ...
  private async handleAnalyzeCommand(args: string[], user: string, channel: string): Promise<void> {
    if (args.length !== 1) {
    ...
    try {
      ...
      await this.sendErrorResponse(channel, `Failed to analyze ${repo}: ${error}`)
    }
  }

  private async handleDeployCommand(args: string[], user: string, channel: string): Promise<void> {
    // Implementation for deployment command
    await this.sendResponse(channel, 'Deployment command received')
  }

  private async handleDebugCommand(args: string[], user: string, channel: string): Promise<void> {
    // Implementation for debug command
    await this.sendResponse(channel, 'Debug command received')
  }

  private async handleStatusCommand(args: string[], user: string, channel: string): Promise<void> {
    // Get system status
  ...
  private async handleNotifyCommand(args: string[], user: string, channel: string): Promise<void> {
    if (args.length < 2) {
      await this.sendUsage(channel, 'notify', 'nikcli notify <channel> <message>')
      return
    }
    ...
    await this.sendResponse(targetChannel, `📢 Notification from ${user}:\n${message}`)
  }

  private async handleWorkflowCommand(args: string[], user: string, channel: string): Promise<void> {
    if (args.length === 0) {
      const availableWorkflows = await this.getAvailableWorkflows()
      await this.sendResponse(channel, `Available workflows:\n${availableWorkflows.join('\n')}`)
      return
    }
    ...
    try {
      ...
      await this.sendResponse(channel, `✓ Workflow '${workflowName}' triggered successfully!`)
    } catch (error) {
      await this.sendErrorResponse(channel, `Failed to trigger workflow: ${error}`)
    }
  }
  ...
  private async getSystemStatus(): Promise<any> {
    return {
      ...
      uptime: '5d 14h',
      activeUsers: 24,
      recentActivity: 'Recent deployments, code reviews, and system health checks',
    }
  }

  private async getAvailableWorkflows(): Promise<string[]> {
    return ['build-and-deploy', 'security-scan', 'performance-test', 'documentation-update']
  }

  private async triggerWorkflow(workflowName: string, context: any): Promise<void> {
  ...
  private async sendErrorResponse(channel: string, error: string): Promise<void> {
    await this.sendResponse(channel, `✖ ${error}`)
  }

  private async sendUsage(channel: string, command: string, usage: string): Promise<void> {
    await this.sendResponse(channel, `Usage: \`${usage}\`\n\nExample: \`/${command} example.com\``)
  }
  ...
  async handleAppMention(event: {
    ...
    const rawPrompt = event.text.replace(/<@[A-Z0-9]+>/, '').trim()
   ...
   */
  private async handleSettingsCommand(channel: string, threadTs: string, session: any): Promise<void> {
...
• \`@nikcli repo=facebook/react branch=main explain the useEffect hook\``
  ...
  private async streamAIResponse(
    session: any,
    prompt: string,
    metadata?: { repo?: string; branch?: string }
  ): Promise<void> {
    try {
      ...
      await aiChatService.generateResponse(session, enrichedPrompt, (delta: string) => {
        accumulatedText += delta

        // Update message every 1 second or 150 chars
        const now = Date.now()
        if (!messageTs || accumulatedText.length % 150 === 0 || now - lastUpdateTime > 1000) {
          this.updateOrPostMessage(session.channel, accumulatedText, session.threadTs, messageTs)
            .then((ts) => {
              messageTs = ts
              lastUpdateTime = now
            })
      ...
      // Final update
      if (accumulatedText) {
        await this.updateOrPostMessage(session.channel, accumulatedText, session.threadTs, messageTs)
