...
function pickAnthropic(
  ...
  try {
    ...
    const anthropicModels = Object.values(allModels)
      .filter((config: any) => config.provider === 'anthropic')
    ...
    for (const model of anthropicModels) {
      const modelTier = classifyAnthropicModel(model.model as string, model.maxContextTokens as number)
...
function pickOpenAI(
  ...
  try {
    ...
    const openaiModels = Object.values(allModels)
      .filter((config: any) => config.provider === 'openai')
      .map((config: any) => config)
    ...
    for (const model of openaiModels) {
      const modelTier = classifyOpenAIModel(model.model as string, model.maxContextTokens as number)
      modelsByTier[modelTier].push(model)
    }
...
function pickGoogle(
  ...
  try {
    ...
    const googleModels = Object.values(allModels)
      .filter((config: any) => config.provider === 'google')
      .map((config: any) => config)
    ...
    for (const model of googleModels) {
      const modelTier = classifyGoogleModel(model.model as string, model.maxContextTokens as number)
      modelsByTier[modelTier].push(model)
    }
