 ...
 * ], inputCode)
 */
export async function sequentialWorkflow<T>(steps: WorkflowStep[], initialInput: T): Promise<WorkflowResult> {
...
export async function parallelWorkflow<T extends Record<string, Promise<any>>>(
  ...
  return Object.fromEntries(entries.map(([key], index) => [key, results[index]])) as { [K in keyof T]: Awaited<T[K]> }
}
...
export async function smartQueryRouter(query: string): Promise<{
  ...
  const systemPrompts: Record<string, string> = {
    ...
    code_review:
      'You are a senior developer reviewing code. Focus on best practices, potential bugs, and improvements.',
...
export async function evaluatorOptimizerWorkflow<T>(config: EvaluatorOptimizerConfig<T>): Promise<{
  finalOutput: T
  iterations: number
