...
Estimate realistic durations and assess risk levels accurately.`
      ...
      try {
        ...
        const plan: ExecutionPlan = {
          ...
          toolCalls: planData.toolCalls.map((tc) => ({
            ...tc,
            id: tc.id || '',
            name: tc.name || '',
  ...
  async executePlan(
    ...
    try {
      ...
      if (
        ...
        plan.toolCalls.some((tc) => tc.name === 'executeCommand')
      ) {
  ...
  getExecutionStats(): {
    ...
    const averageExecutionTime =
      totalToolCalls > 0 ? allResults.reduce((sum, r) => sum + r.executionTime, 0) / totalToolCalls : 0
