...
import { BaseAgent, type AgentMetrics, type AgentTaskResult } from './base-agent'
import type { AgentTask } from './agent-router'
import {
    type AgentPerformanceMetrics,
    type CognitiveCapabilities,
...
export abstract class CognitiveAgentBase extends BaseAgent implements CognitiveCapabilities {
    ...
    private calculateTaskComplexity(intent: TaskCognition['intent'], entities: TaskCognition['entities'], dependencies: string[]): number {
        let complexity = 5 // Base complexity

        // Intent complexity
        const intentComplexity = {
            read: 2, create: 6, update: 5, delete: 3,
            analyze: 7, optimize: 8, deploy: 9, test: 4, debug: 7, refactor: 8
        }
    ...
    private assessRiskLevel(intent: TaskCognition['intent'], entities: TaskCognition['entities'], complexity: number): 'low' | 'medium' | 'high' {
        let riskScore = 0
        ...
        const criticalFiles = entities.filter(e =>
            e.name.includes('package.json') ||
    ...
    private determineRequiredCapabilities(intent: TaskCognition['intent'], entities: TaskCognition['entities']): string[] {
    ...
    private suggestOptimalAgents(requiredCapabilities: string[], complexity: number): string[] {
        ...
        for (const [agentId, agentCaps] of Object.entries(agentCapabilityMap)) {
            if (agentCaps.includes('*') ||
                requiredCapabilities.some(req => agentCaps.includes(req))) {
    ...
    private async createOrchestrationPhases(cognition: TaskCognition, strategy: OrchestrationPlan['strategy']): Promise<OrchestrationPhase[]> {
    ...
    private calculateResourceRequirements(cognition: TaskCognition, phases: OrchestrationPhase[]): OrchestrationPlan['resourceRequirements'] {
    ...
    private updateCognitiveMetrics(cognition: TaskCognition, success: boolean, duration: number): void {
        ...
        this.cognitiveMetrics.userSatisfaction = Math.max(0, Math.min(1,
            this.cognitiveMetrics.userSatisfaction + satisfactionDelta + efficiencyBonus
        ))
    }
    ...
    private analyzeFailurePatterns(taskHistory: TaskCognition[]): string[] {
        ...
        const recentFailures = this.cognitiveMemory.performanceHistory
            .filter(h => !h.success)
            .slice(-20)
    ...
    private analyzeResourceEfficiency(): string[] {
        ...
        if (this.cognitiveMetrics.averageDuration > 300) { // 5 minutes
