...
export class ReactAgent extends CognitiveAgentBase {
  id = 'react'
  capabilities = [
    'react', 'tsx', 'frontend', 'components', 'nextjs', 'typescript',
    'hooks', 'state-management', 'performance-optimization', 'component-analysis',
    'testing-strategy', 'ssr-ssg', 'accessibility', 'responsive-design',
  ]
  ...
  protected cognitiveStrengths = [
    'Component architecture analysis', 'Performance optimization',
    'Modern React patterns', 'TypeScript integration',
    'Next.js SSR/SSG optimization', 'State management strategies',
  ]
  protected cognitiveWeaknesses = [
    'Complex backend integration', 'Low-level system operations',
  ...
  protected getSpecializationOptimizations(): string[] {
    ...
    if (componentPatterns.length > 10) {
      const avgComplexity = componentPatterns.reduce((sum, p) => sum + p.estimatedComplexity, 0) / componentPatterns.length
      if (avgComplexity > 7) {
  ...
  private async initializeReactCognition(): Promise<void> {
    const reactPatterns = [
      'component-creation', 'hook-implementation', 'state-management-setup',
      'performance-optimization', 'testing-setup'
    ]
  ...
  private async generateIntelligentSolution(cognition: TaskCognition, context: any, componentAnalysis: any): Promise<any> {
...
Provide complete, working code that can be used immediately.`
  ...
  private async processResponse(response: string, cognition: TaskCognition, componentAnalysis: any, context: any): Promise<any> {
    const result = {
