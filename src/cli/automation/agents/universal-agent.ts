...
`;
  ...
  private cleanupMemory(): void {
    ...
    for (const [id, plan] of this.activeOrchestrations.entries()) {
      if (plan.estimatedDuration < Date.now() - 3600000) { // 1 hour old
        this.activeOrchestrations.delete(id);
      }
    }

    console.log(`🧹 Memory cleanup: ${this.cognitiveMemory.length} cognitions, ${this.orchestrationHistory.length} history`);
  }
