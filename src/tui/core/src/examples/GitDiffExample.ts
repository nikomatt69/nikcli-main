...
import { GitDiffBlock, FileChangeTracker } from '../components';
...
export class GitDiffExample {
  ...
  private setupEventHandlers(): void {
    ...
    this.tracker.subscribe((event) => {
      this.updateStatusBar(`${event.operation} - ${event.filePath} (${event.diff?.stats.additions}+, ${event.diff?.stats.deletions}-)`);
