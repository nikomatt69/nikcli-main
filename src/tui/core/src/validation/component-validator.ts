...
export class ComponentValidator {
  ...
  public validateComponent<T extends keyof typeof ComponentSchemas>(componentName: T, props: any): ValidationResult {
    try {
  ...
  private checkUnknownProperties(componentName: keyof typeof ComponentSchemas, props: any): string[] {
    ...
    Object.keys(props).forEach((prop) => {
      if (!validProps.includes(prop)) {
        warnings.push(`Unknown property '${prop}' for component '${componentName}'`)
      }
    })
  ...
  public validateComponents(components: Array<{ name: string; props: any }>): ValidationResult[] {
    return components.map((component) =>
      this.validateComponent(component.name as keyof typeof ComponentSchemas, component.props)
    )
  }

  // Get validation schema for a specific component
  public getSchema<T extends keyof typeof ComponentSchemas>(componentName: T): z.ZodSchema {
    return this.schemas[componentName]
  }

  // Check if a component has a specific property
  public hasProperty<T extends keyof typeof ComponentSchemas>(componentName: T, propertyName: string): boolean {
    const schema = this.schemas[componentName]
        ...
        switch (issue.code) {
          case 'invalid_type':
            if (issue.received === 'undefined') {
              suggestions.push(`Property '${issue.path.join('.')}' is required`)
            } else {
              suggestions.push(`Property '${issue.path.join('.')}' should be of type ${issue.expected}`)
            }
            break
          case 'invalid_string':
            if (issue.validation === 'regex') {
              suggestions.push(`Property '${issue.path.join('.')}' has invalid format`)
            }
            break
          case 'too_small':
            suggestions.push(`Property '${issue.path.join('.')}' is too small (minimum: ${issue.minimum})`)
            break
          case 'too_big':
            suggestions.push(`Property '${issue.path.join('.')}' is too big (maximum: ${issue.maximum})`)
            break
          case 'invalid_enum_value':
            suggestions.push(`Property '${issue.path.join('.')}' must be one of: ${issue.options.join(', ')}`)
            break
        }
      })
    }
...
export const validateComponent = <T extends keyof typeof ComponentSchemas>(componentName: T, props: any) =>
  componentValidator.validateComponent(componentName, props)

export const validateComponentStrict = <T extends keyof typeof ComponentSchemas>(componentName: T, props: any) =>
  componentValidator.validateComponentStrict(componentName, props)

export const validateWithSuggestions = <T extends keyof typeof ComponentSchemas>(componentName: T, props: any) =>
