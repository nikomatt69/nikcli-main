...
import { type ComponentSize, type ComponentState, type ComponentVariant, tokens } from '../theming/design-tokens'
...
export function createResponsiveVariants<T extends ComponentConfig>(config: T) {
  return function getResponsiveVariants(props: VariantProps<{ responsive?: Record<string, any> }> = {}) {
...
export function createCompoundVariants<T extends ComponentConfig>(
  ...
  return function getCompoundVariants(props: VariantProps<{}> = {}) {
    ...
    const compoundClass =
      compoundVariants.find((cv) =>
        Object.entries(cv.variants).every(([key, value]) => props[key as keyof VariantProps<{}>] === value)
      )?.className || ''
...
export function createContextVariants<T extends ComponentConfig>(config: T, contextKey: string) {
