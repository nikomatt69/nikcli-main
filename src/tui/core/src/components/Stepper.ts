  ...
  private renderSteps() {
    ...
    this.steps.forEach((step, index) => {
      ...
      const stepElement = blessed.text({
        ...
        style: {
          fg: isActive ? this.theme.accent : isCompleted ? 'green' : this.theme.foreground,
        },
      })
      ...
      if (isClickable) {
        ...
        stepElement.on('mouseout', () => {
          const isActive = index === this.activeIndex
          const isCompleted = step.completed
          stepElement.style.fg = isActive ? this.theme.accent : isCompleted ? 'green' : this.theme.foreground
          this.el.screen.render()
        })
      }
