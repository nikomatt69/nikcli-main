...
import { resolveTheme, type StyleProps, type Theme } from '../theming/theme'
...
export function createBoxBase<T extends Widgets.BoxElement = Widgets.BoxElement>(
  ...
  const baseStyle = computeBlessedStyle(theme, opts, componentName, variant, size)

  // Merge styles
  const mergedStyle = mergeComponentStyles(baseStyle, componentTokens, {}, opts.blessedProps?.style || {})
...
function layoutComponentsInGrid<T extends Widgets.BlessedElement = Widgets.BlessedElement>(
  ...
  const _rows = Math.ceil(components.length / cols)
