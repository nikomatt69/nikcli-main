...
export const PaddingSchema = z
  .union([
    ...
    z
      .tuple([z.number().min(0), z.number().min(0), z.number().min(0), z.number().min(0)])
...
export const TerminalColorSchema = z.union([
  ...
  z
    .string()
    .regex(/^rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)$/, "RGB must be in format 'rgb(r,g,b)'"),
  // 256-color palette
  z
    .number()
    .min(0)
    .max(255)
    .describe('256-color palette index'),
])
...
export const TokenColorReferenceSchema = z.union([
  ...
  z
    .string()
    .regex(/^[a-zA-Z][a-zA-Z0-9]*\.[a-zA-Z][a-zA-Z0-9]*\.[a-zA-Z0-9]+$/, "Token reference: 'namespace.token.variant'"),
])

/**
 * Combined color schema
 */
export const ColorSchema = z.union([TerminalColorSchema, TokenColorReferenceSchema])
 ...
 */
export const AnimationTypeSchema = z.enum(['none', 'fade', 'slide', 'bounce', 'pulse', 'spin']).optional()
 ...
 */
export const ComponentStateSchema = z.enum(['default', 'hover', 'focus', 'active', 'disabled', 'loading', 'selected'])
 ...
 */
export const AccessibilityPropsSchema = z
  .object({
    role: AriaRoleSchema,
...
export const ButtonPropsSchema = BasePropsSchema.extend({
  ...
  .merge(EventPropsSchema.unwrap())
  .merge(AccessibilityPropsSchema.unwrap())
  .strict()
...
export const TablePropsSchema = BasePropsSchema.extend({
  ...
  onRowSelect: z
    .function()
    .args(z.object({ row: z.any(), index: z.number() }))
    .returns(z.void())
    .optional(),
  onSort: z
    .function()
    .args(z.object({ column: z.string(), direction: z.enum(['asc', 'desc']) }))
    .returns(z.void())
    .optional(),
})
  .merge(EventPropsSchema.unwrap())
...
export const FormPropsSchema = BasePropsSchema.extend({
  ...
  onFieldChange: z
    .function()
    .args(z.object({ name: z.string(), value: z.any() }))
    .returns(z.void())
    .optional(),
})
  .merge(EventPropsSchema.unwrap())
...
export const ListPropsSchema = BasePropsSchema.extend({
  ...
  onSelect: z
    .function()
    .args(z.object({ item: z.any(), index: z.number() }))
    .returns(z.void())
    .optional(),
  onSelectionChange: z
    .function()
    .args(z.object({ items: z.array(z.any()), indices: z.array(z.number()) }))
    .returns(z.void())
    .optional(),
})
  .merge(EventPropsSchema.unwrap())
