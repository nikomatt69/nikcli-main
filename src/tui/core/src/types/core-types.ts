 ...
 */
export type EventHandler<T = void> = T extends void ? () => void : (value: T) => void
 ...
 */
export type GetBlessedElement<T extends BlessedWidgetType> = T extends keyof BlessedElementMap
  ? BlessedElementMap[T]
 ...
 */
export type TerminalUnit = number | `${number}%` | 'center' | 'left' | 'right' | 'top' | 'bottom'
 ...
 */
export type TokenColorReference = `${string}.${string}` | `${string}.${string}.${string}`
 ...
 */
export type BorderStyle = 'line' | 'bg' | 'none' | { type: 'line' | 'bg'; fg?: ColorValue }
 ...
 * Component states
 */
export type ComponentState = 'default' | 'hover' | 'focus' | 'active' | 'disabled' | 'loading' | 'selected'

/**
 * Animation types for components
 */
export type AnimationType = 'none' | 'fade' | 'slide' | 'bounce' | 'pulse' | 'spin'
 ...
 */
export interface ComponentInstance<TElement extends Widgets.BlessedElement = Widgets.BlessedElement> {
...
export interface FormField<T = any> {
  ...
  readonly type: 'text' | 'number' | 'email' | 'password' | 'textarea' | 'select' | 'checkbox' | 'radio'
  readonly required?: boolean
  readonly placeholder?: string
  readonly defaultValue?: T
  readonly validate?: (value: T) => ValidationResult | Promise<ValidationResult>
 ...
 * Form data type
 */
export type FormData<T extends Record<string, FormField> = Record<string, FormField>> = {
...
export interface InputSpecificProps extends EventProps, AccessibilityProps {
  ...
  readonly validate?: (value: string) => ValidationResult | Promise<ValidationResult>
}

/**
 * List specific props
 */
export interface ListSpecificProps<T = any> extends EventProps, AccessibilityProps {
 ...
 */
export type ComponentProps<T> = T extends ComponentConstructor<infer P, any> ? P : never

/**
 * Extract element type from component constructor
 */
export type ComponentElement<T> = T extends ComponentConstructor<any, infer E> ? E : never

/**
 * Polymorphic component props
 */
export type PolymorphicProps<T extends BlessedWidgetType> = BaseComponentProps & {
  readonly as?: T
