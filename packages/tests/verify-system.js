...
async function main() {
  ...
  await verifier.test('Project Structure', async () => {
    const requiredPaths = ['src', 'src/cli', 'src/cli/index.ts', 'tests', 'package.json', 'tsconfig.json']
  ...
  await verifier.test('Security Validation', async () => {
    ...
    for (const cmd of dangerousCommands) {
      const isDangerous = cmd.includes('rm -rf /') || cmd.includes('format ') || cmd.includes('del /f /s /q')
  ...
  await verifier.test('Performance Validation', async () => {
    ...
    const operations = []
    for (let i = 0; i < 100; i++) {
      operations.push(fs.writeFile(`perf-test-${i}.txt`, `Content ${i}`))
    }
    ...
    const readOps = []
    for (let i = 0; i < 100; i++) {
      readOps.push(fs.readFile(`perf-test-${i}.txt`, 'utf-8'))
    }
