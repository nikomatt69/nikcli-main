...
describe('System Health Check - End to End', () => {
    ...
    it('should build the CLI successfully', async () => {
      try {
        ...
        const distExists = await fs
          .access(path.join(testWorkspace, '../dist'))
          .then(() => true)
          .catch(() => false)
  ...
  describe('File System Operations', () => {
    ...
    it('should handle concurrent operations', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        operations.push(fs.writeFile(`concurrent-${i}.txt`, `Content ${i}`))
      }
  ...
  describe('Security and Validation', () => {
    it('should validate file paths', () => {
      const safePaths = ['file.txt', 'folder/file.txt', 'deep/nested/file.txt']

      const unsafePaths = ['../outside.txt', '/absolute/path.txt', 'file\x00.txt']
    ...
    it('should handle command validation', () => {
      const safeCommands = ['ls -la', 'npm install', 'git status', 'node script.js']

      const dangerousCommands = ['rm -rf /', 'format C:', 'sudo rm -rf /', 'del /f /s /q C:\\']

      // Safe commands should pass basic validation
      for (const cmd of safeCommands) {
        const hasDangerousPattern = cmd.includes('rm -rf /') || cmd.includes('format ') || cmd.includes('del /f /s /q')
  ...
  describe('Integration Health Check', () => {
    it('should verify all critical components are accessible', async () => {
      ...
      const requiredPaths = ['../src', '../src/cli', '../package.json', '../tsconfig.json']
