...
import { useTerminal, Modal, Button } from "nikcli-tui";
...
import { Scrollable, Text } from "nikcli-tui";
...
fileList.setItems([
  { name: "src", icon: "📁", isDirectory: true, size: "4 KB" },
  { name: "package.json", icon: "📄", isDirectory: false, size: "2 KB" },
  { name: "README.md", icon: "📄", isDirectory: false, size: "5 KB" },
]);
```
...
import { Tooltip, Button } from "nikcli-tui";
...
tooltipManager.attach(complexButton, {
  ...
  content:
    "Access advanced AI capabilities including code analysis and generation",
  position: "right",
  width: 40,
});
```
...
class ContextualHelpSystem {
  ...
  showHelp(context?: string) {
    ...
    const helpContent =
      this.helpData.get(contextKey) || this.helpData.get("default")!;
...
class SetupWizard {
  ...
  private updateNavigation() {
    ...
    this.nextButton.setContent(isLast ? "Finish" : "Next →");
  }
}
```
...
<CardGroup cols={2}>
  ...
  <Card title="Agent System" icon="robot" href="/agents/introduction">
  ...
  <Card title="Component Overview" icon="palette" href="/components/overview">
